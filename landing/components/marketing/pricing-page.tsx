...
export function PricingPage({ locale }: { locale: Locale }) {
  ...
  return <div className={c("page-shell")}>
    ...
    <header className={c("site-header", scrolled && "scrolled")} id="top"><nav className={c("nav", "container")} aria-label={locale === "vi" ? "Điều hướng chính" : "Primary navigation"}>
      <Link className={c("brand")} href={prefix || "/"} aria-label="ManleAI home"><img src="/brand/manle-ai-logo-720.png" alt="ManleAI logo" width="54" height="54" /><span className={c("brand-wordmark")}>Manle<span>.ai</span></span></Link>
    ...
    <main id="pricing-main">
      ...
      <section className={c("pricing-truth")}><div className={c("container")}><span className={c("kicker", "light")}>{locale === "vi" ? "Sẵn sàng đặt lịch" : "Booking readiness"}</span><h2>{locale === "vi" ? "Salon luôn kiểm soát lịch hẹn." : "Calendar control stays with your salon."}</h2><p>{locale === "vi" ? "Tài khoản mới bắt đầu ở chế độ chỉ ghi nhận yêu cầu. ManleAI Calendar chỉ thực hiện lịch hẹn sau khi thiết lập được hoàn tất, review và kích hoạt. Square Appointments cần hoàn tất kết nối và thiết lập booking riêng." : "New accounts begin in request-only mode. ManleAI Calendar handles appointments only after setup is completed, reviewed and activated. Square Appointments requires its own completed connection and booking setup."}</p></div></section>
    </main>
