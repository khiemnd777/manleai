...
export function PlatformAuditLog({ tenantID }: { tenantID: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const response=await apiRequest<{data:AuditEvent[];meta:{page:{limit:number;offset:number;has_more:boolean}}}>(`/api/v2/platform/tenants/${encodeURIComponent(tenantID)}/audit-events?limit=100`); setResult({events:response.data,...response.meta.page}); }
