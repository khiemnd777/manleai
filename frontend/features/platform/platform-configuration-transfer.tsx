...
export function PlatformConfigurationTransfer({ tenantID }: { tenantID: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-start gap-3"><div className="flex h-10 w-10 flex-none items-center justify-center rounded-md bg-teal-50 text-brand"><FileJson className="h-5 w-5" /></div><div><CardTitle>1. Choose a source</CardTitle><CardDescription>Use another authorized tenant, a v9 export, a compatibility v8 JSON file, or a scoped content-only v7 pack.</CardDescription></div></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          <SourceChoice active={sourceType === "json_upload"} disabled={Boolean(busy)} title="JSON upload" description="Supports v9, v8, and safely adapted scoped v7 content packs." onClick={() => changeSourceType("json_upload")} />
        </div>
