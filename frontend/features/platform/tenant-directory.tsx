...
export function PlatformTenantDirectory() {
  ...
  const filtered = useMemo(() => {
    ...
    return salons.filter((salon) => [salon.name, salon.city, salon.state, salon.public_slug].some((value) => value?.toLowerCase().includes(needle)));
  }, [query, salons]);

  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-bold text-ink">Nail salons</h1><p className="mt-1 text-sm text-muted">Open one salon to manage its Business, Technical, Operations, and Audit workspaces.</p></div><Button type="button" variant="secondary" onClick={() => void load()} disabled={loading}><RefreshCcw className="h-4 w-4" />Refresh</Button></div>
      <Card><label className="block"><span className="text-sm font-semibold text-ink">Search salons</span><div className="mt-2 flex h-11 items-center gap-2 rounded-md border border-line bg-white px-3"><Search className="h-4 w-4 text-muted" /><input className="w-full bg-transparent text-sm outline-none" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, city, state, or public slug" /></div></label></Card>
      ...
      {!loading && !error && filtered.length ? <div className="overflow-hidden rounded-lg border border-line bg-white shadow-soft"><div className="hidden grid-cols-[minmax(0,2fr)_minmax(0,1fr)_160px_48px] gap-4 border-b border-line bg-slate-50 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-muted md:grid"><span>Salon</span><span>Location</span><span>Access</span><span /></div>{filtered.map((salon) => <Link key={salon.id} href={`/platform/tenants/${salon.id}/business`} className="grid gap-3 border-b border-line px-5 py-4 transition last:border-0 hover:bg-slate-50 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_160px_48px] md:items-center"><div className="min-w-0"><div className="truncate font-semibold text-ink">{salon.name}</div><div className="mt-1 truncate text-xs text-muted">{salon.public_slug ? `/s/${salon.public_slug}` : "Public page not configured"}</div></div><div className="text-sm text-slate-700">{[salon.city, salon.state].filter(Boolean).join(", ") || salon.timezone}</div><div><Badge value={salon.business_access} /></div><ChevronRight className="hidden h-4 w-4 text-muted md:block" /></Link>)}</div> : null}
    </div>
  );
}
