...
export function BusinessStaffManager({ surface, title = "Staff" }: { surface: BusinessSurface; title?: string }) {
  ...
  async function archive(item: BusinessStaff) {
    ...
    try { const response=await businessMutation<BusinessStaff>(surface,`staff/${item.id}/archive`,"POST",{action_key:key,expected_version:item.version});setStaff((items)=>upsert(items,response.data));setSuccess("Staff member archived."); }
