...
const platformNav = [
  ...
  { label: "Access control", href: "/platform/access", icon: KeyRound }
];

function PlatformNavigation({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-shell">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-line bg-white px-4 py-5 lg:block">
        <div className="flex h-full flex-col">{brand}<nav className="mt-8 space-y-1"><PlatformNavigation pathname={pathname} /></nav><div className="mt-auto border-t border-line pt-4"><Button type="button" variant="ghost" className="w-full justify-start" onClick={logout}><LogOut className="h-4 w-4" />Sign out</Button></div></div>
      </aside>
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true" aria-label="Platform navigation">
          ...
          <div className="relative z-10 flex h-full w-80 max-w-[85vw] flex-col border-r border-line bg-white px-4 py-5 shadow-soft">
            ...
            <nav className="mt-8 space-y-1"><PlatformNavigation pathname={pathname} onNavigate={() => setMobileOpen(false)} /></nav>
