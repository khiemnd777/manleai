...
7. For provider setup or active-runtime diagnosis, treat Platform-managed
   ...
   Runtime Square/Twilio/OpenAI resolution now enters that legacy bootstrap path
   only for exact repository `ErrNotFound`; repository, malformed settings, and
   decryption errors fail closed. A stored row never inherits environment
   credentials or enabled state. Route future changes through
...
| --- | --- | --- | --- |
| Auth and sessions | `backend/modules/auth/*`, `backend/internal/middleware/auth.go`, `frontend/lib/api/browser-session.ts`, `pos-calendar/lib/api/browser-session.ts` | Generic non-enumerating login for invalid and disabled accounts; host-only HttpOnly/Secure-production/SameSite-Strict refresh cookie scoped to `/api/auth`; memory-only browser access tokens plus deletion of legacy local-storage keys; deterministic one-successor active-user rotation with bounded exact concurrent replay and hashed persistence; bootstrap owner; shared `RequireAuth` reloads active server-owned user/salon/roles and fails closed with generic `401 UNAUTHENTICATED` | auth service/handler/repository integration tests, auth middleware tests, and both browser-session evidence tests |
| SaaS access control | `backend/modules/access/*`, `backend/internal/middleware/auth.go`, `backend/cmd/platform-access/main.go`, `backend/migrations/V64__saas_access_control_foundation.sql` | Expand-only tenant memberships and owner backfill/synchronization; current Platform Admin/Ops roles; route-owned Tenant/Platform surfaces; exact-salon Ops capability assignment; Platform PII default deny with maximum-24-hour exact-scope grant and bounded opaque change reference; serialized role governance with concurrent last-admin protection and atomic child-delegation revocation; expected-version CAS; action-key/fingerprint exact replay; immutable safe audit; one-time first-admin bootstrap; owner-membership guards | `backend/modules/access/*_test.go`, `backend/internal/middleware/auth_test.go`, `backend/modules/auth/repository_integration_test.go`, `backend/migrations/v64_safety_test.go` |
| SaaS shared Business management | `backend/modules/business/*`, `backend/migrations/V65__saas_business_management.sql`, `backend/modules/salon/repository.go` public-catalog readiness helper | Fixed Tenant `/api/salons/:id/business/*` and Platform `/api/platform/tenants/:tenant_id/business/*` surfaces over the same canonical salon profile, services/categories/consultation profiles, staff/service eligibility, authority-aware hours, public page settings, and customers; membership/capability authorization before repository reads; customer PII grants on Platform reads/writes; Platform staff contacts omitted and unwritable; provider-linked operational fields and external hours read-only; exact action-key replay, expected resource versions, actual-actor audit, shared scheduling fence, safe audit allowlists, provider/legacy insert version triggers; canonical public readiness and POS consultation validation reused rather than duplicated | `backend/modules/business/*_test.go`, `backend/migrations/v65_safety_test.go`, `backend/modules/salon/public_catalog_integration_test.go` |
...
| SaaS runtime isolation and fairness | `backend/internal/database/context_connector.go`, `backend/internal/database/runtime_role.go`, `backend/internal/databasecontext/*`, `backend/modules/tenant_runtime/*`, `backend/migrations/V67__tenant_runtime_membership_boundary.sql`, `V68__saas_row_level_security.sql`, `V69__tenant_runtime_quotas_usage.sql`, `V71__public_catalog_projection_boundary.sql`, `V72__platform_pii_scope_enforcement.sql` | Request actor/system scope is applied per DB operation and reset before connection reuse; production runtime role must be non-owner/non-superuser/non-BYPASSRLS; tenant row isolation; active-membership runtime gate; public zero base-table visibility with safe JSON projection; exact Platform PII scopes across customer/call/appointment/notification tables; database-owned tenant quotas/usage plus fair worker claim caps | `backend/internal/database/*_test.go`, `backend/modules/tenant_runtime/*_test.go`, `backend/migrations/v67_safety_test.go` through `v72_safety_test.go` |
...
| Integration config | `backend/modules/integration_config/*` | Authenticated salon-scoped Square/Twilio/OpenAI runtime settings and encrypted secrets; exact-`ErrNotFound`-only legacy bootstrap resolution; fail-closed repository/settings/decryption handling; stored-row authority over provider enabled state and credentials; dashboard secret-source redaction without environment inheritance; strict string-valued settings-object scanning; dashboard-managed owner-SMS consent/destination/sender/callback settings; whole-response write-only secret/SID/destination serialization contract; strict database-only `ResolveTwilioMessagingConfig`; narrow database-only `ResolveStoredTwilioAuthToken` for voice audio signing without messaging readiness coupling; and fail-closed `ResolveOpenAIConfigStrict` for workflows that must not use legacy environment fallback | `backend/modules/integration_config/routes_test.go`, `backend/modules/integration_config/service_test.go`, `backend/modules/integration_config/repository_test.go`, caller propagation tests in `backend/modules/pos_square/adapter_test.go` and `backend/modules/voice/service_test.go` |
| Configuration transfer | `backend/modules/config_transfer/*` | Owner-first schema v8 export/import preview/apply with v7 compatibility, scoped `included_sections` data packs, exact destination authority/version fence, explicit adapter-intent reporting, stable request IDs, authority-aware consultation/public readiness, and exclusion of secrets/scheduling/internal-execution/operational records | `backend/modules/config_transfer/*_test.go` |
...
| `/dashboard/services`, `/dashboard/staff`, `/dashboard/customers`, `/dashboard/settings` | `frontend/features/business/tenant-business-page.tsx`, `business-services.tsx`, `business-staff.tsx`, `business-customers.tsx`, `business-settings.tsx` | Tenant Business objects only through `/api/salons/:id/business/*`; no provider secrets, provider setup, scheduling authority switch, technical readiness, or Operations controls |
...
| `/dashboard/calls` | `frontend/features/business/tenant-calls-console.tsx` | Tenant call Business view scoped to current membership |
...
| `/platform/access` | `frontend/features/platform/access-console.tsx` | Platform Admin role/membership/delegation/PII-grant management |
| `/platform/tenants/[tenantId]/business` | `frontend/features/platform/platform-business-workspace.tsx` plus shared Business editors | Platform operator manages the same canonical tenant Business objects with actual-actor audit |
| `/platform/tenants/[tenantId]/technical` | `frontend/features/platform/technical-integration-settings.tsx`, `technical-scheduling-settings.tsx` | Platform-only Square/Twilio/OpenAI, AI runtime, ManleAI Calendar, and scheduling-authority technical controls |
...
| `/platform/tenants/[tenantId]/audit` | `frontend/features/platform/platform-audit-log.tsx` | Tenant-scoped access/Business/Technical/Operations immutable audit projections |
...
The table below records the previous Owner-first dashboard composition for
regression archaeology only. It is not current route ownership; use the SaaS
table above for implementation and triage.
...
| `/dashboard/settings` | `frontend/app/dashboard/settings/page.tsx` | `frontend/features/dashboard/settings-dashboard.tsx`, `frontend/features/dashboard/internal-calendar-setup.tsx`, `frontend/features/dashboard/customer-sms-policy.tsx`, `frontend/features/dashboard/scheduling-readiness-card.tsx`, `frontend/features/dashboard/operations-health.tsx` | salon profile/settings plus Phase 3 internal policy, `local_override` hours, resource pools, salon/resource exceptions, version-fenced activation audit, constraints/readiness, default-off customer SMS policy/quiet hours, selected-authority consultation eligibility without a universal POS-link gate, and read-only Phase 6B recurring-job/tenant-queue health with safe workflow links |
...
- Data owner: V64 `salon_memberships`, `platform_role_assignments`,
  ...
  `permissions.delegation_scope`.
- Fixed invariants: `salons.owner_user_id` remains owner source of truth;
  owner membership is active and non-revocable through Platform access APIs;
  the last active Platform Admin remains; Platform Ops is exact-salon and
  exact-capability scoped; every Platform actor needs an exact active grant for
  PII; grants last no more than 24 hours; access actions are replay-safe and
  audit rows immutable.
- Operator bootstrap owner: `backend/cmd/platform-access/main.go`; it works only
  for one exact existing active user while no active Platform Admin exists.
...
- Runtime/database owner: `backend/internal/databasecontext`, the contextual
  ...
  V67-V72. RLS is tenant defense-in-depth, public reads use only V71, and
  Platform PII reads require the exact V72 scope.
...
- Frontend owner: shared `frontend/features/business/*` editors under the
  Tenant Business pages and Platform tenant Business workspace.
...
- Data owner: sanitized schema v8 import/export DTOs with v7 compatibility, including portable consultation profiles that resolve to existing target services. Scheduling authority/version and history, scheduling requests/outbox rows, internal calendar configuration/execution evidence, service records, provider mappings/connections, secrets, and operational records are excluded. `active_pos_provider` is reported adapter intent and never authority consent. Provider-neutral profile normalization and idempotent transaction writes remain owned by `backend/modules/pos`.
- Concurrency boundary: apply acquires `backend/modules/scheduling/fence` and rechecks the previewed `salon_settings.scheduling_authority` plus version before any write. External-executor adapter changes are explicit provider-switch blockers.
- Tests: `backend/modules/config_transfer/*_test.go`, including real-PostgreSQL replay, tenant, and authority-fence coverage.
...
| service alias ownership, category alias ownership, cross-table alias conflict, duplicate normalized alias, alias reactivation, alias deactivation transfer, V58 | `backend/migrations/V58__cross_table_service_alias_ownership.sql`, `backend/modules/training/repository.go`, `backend/modules/pos/repository.go` | salon/normalized-alias transaction advisory lock, symmetric database triggers, config-transfer alias upserts, taxonomy materialization, V58 static safety test and real PostgreSQL parallel invariant test |
...
| AI training, owner correction, knowledge, FAQ answer, stale policy, training evaluation, pos_confirmation_required, confirmation_requirement, confirmation_guardrail, universal Square training claim | `backend/modules/training`, `backend/modules/scheduling/service.go`, answer router/context | owner-scoped `CurrentSchedulingAuthority`, Training UI, API contract, knowledge and authority guardrail tests |
| configuration transfer, configuration import, schema v8, v7 compatibility, included_sections, active_pos_provider intent, source adapter, destination adapter, result adapter, source booking mode, result booking mode, confirmed_booking_incompatible_with_owner_manual, authority fence, target_scheduling_authority_version, concurrent authority switch, consultation profile import, webhook URL preserve, signature secret re-entry, profile target missing, profile target ambiguous, repeated import | `backend/modules/config_transfer/service.go`, `backend/modules/config_transfer/repository.go`, `backend/modules/config_transfer/types.go`, `backend/modules/scheduling/fence` | authority/version-preserving apply, explicit owner-manual pending-mode decision, authority-aware consultation/public readiness, external adapter-switch blocker, provider deployment URL preservation, repository transaction/upserts, provider-neutral profile helpers in `backend/modules/pos`, Settings/onboarding preview UI, config transfer unit/PostgreSQL tests |
...
| archive, call-session redaction, transcript PII, dialog_state PII, party_plan PII, booking_segments PII, reschedule_candidates PII, historical redaction repair, idempotent re-redaction, call audio cleanup | `backend/modules/conversation/repository.go`, `backend/modules/conversation/retention_processor.go` | owner/tenant-fenced lifecycle actions; complete state/transcript/handoff/party/webhook/audio clearing; `backend/modules/conversation/retention_integration_test.go`; Calls UI lifecycle filters and worker |
| scheduling PII retention, notification retention, V61, retention_expires_at, redacted_at, redaction_version, 90-day boundary, terminal business state, terminal delivery state, SKIP LOCKED, redacted owner request, redacted destination, expired voice audio, retention allowlist, irreversible redaction | `backend/modules/scheduling_retention`, `backend/migrations/V61__scheduling_pii_retention.sql` | owner-manual/owner-notification/customer-notification retained DTOs, V61 database guards, worker and operations-health registration, owner-review/delivery UI markers, PostgreSQL integration and migration safety tests |
| SaaS tenant, multi-tenant, ActorContext, tenant membership, tenant_owner, tenant_business_manager, Platform Admin, platform_admin, Platform Ops, platform_ops, salon assignment, delegated capability, delegation_scope, PII grant, customers scope, calls scope, appointments scope, notifications scope, 24-hour grant, access action, access audit, Technical tab, Operations tab, runtime role, RLS, noisy neighbor, fair worker, tenant quota, public projection, V64, V65, V66, V67, V68, V69, V70, V71, V72, cross-tenant Platform access, impersonation header, first Platform Admin bootstrap, last admin | `backend/modules/access`, `backend/modules/business`, Platform handlers, `backend/modules/tenant_runtime`, `backend/internal/database`, `backend/internal/middleware/auth.go`, `backend/migrations/V64*` through `V72*`, `backend/cmd/platform-access/main.go` | fixed Tenant/Platform routes; current principal resolution; exact salon/capability/PII-grant policy; owner synchronization; version/idempotency/audit; non-owner runtime role and RLS; public safe projection; quota/fairness; full SaaS frontend route map |
