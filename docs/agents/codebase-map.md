...
| `/platform/tenants/[tenantId]/calls` | shared `frontend/features/dashboard/calls-dashboard.tsx` with `surface="platform"` | Calls, party requests, readiness, catalog projections, and corrections use v2 Calls resources; Platform Admin is direct while Ops retains exact authorization plus Calls PII. |
