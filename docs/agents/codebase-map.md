...
| Twilio Voice adapter | `backend/modules/voice_twilio/*` | Twilio signatures, form parsing, TwiML, recording mode, Media Streams bridge, caller-input gate, typed reply scheduling, stale-generation suppression, recovery first-byte budget, terminal latch | `backend/modules/voice_twilio/*_test.go` |
...
| Integration config | `backend/modules/integration_config/*` | Salon-scoped Square/Twilio/OpenAI runtime settings and encrypted secrets; exact-`ErrNotFound`-only legacy bootstrap resolution; fail-closed repository/settings/decryption handling; stored-row authority over provider enabled state and credentials; dashboard secret-source redaction without environment inheritance; strict string-valued settings-object scanning; dashboard-managed owner-SMS consent/destination/sender/callback settings; write-only Twilio Account/Auth/Messaging Service data; strict database-only `ResolveTwilioMessagingConfig`; narrow database-only `ResolveStoredTwilioAuthToken` for voice audio signing without messaging readiness coupling; and fail-closed `ResolveOpenAIConfigStrict` for workflows that must not use legacy environment fallback | `backend/modules/integration_config/service_test.go`, `backend/modules/integration_config/repository_test.go`, caller propagation tests in `backend/modules/pos_square/adapter_test.go` and `backend/modules/voice/service_test.go` |
...
| Public catalog API | `backend/modules/public_catalog/*` | Public-safe selected-authority catalog reads; owner-manual canonical services without POS/staff gates, current activated internal catalog/local hours, current synced external projections, stale-publication fail-closed behavior, authority/version exposure, and no provider identifiers or confirmation action | `backend/modules/public_catalog/service_test.go`, `backend/modules/salon/public_catalog_integration_test.go` |
...
- Data owner: public-safe salon profile plus the services/staff/hours eligible
  for the persisted scheduling authority; the public flag is not sufficient
  when current readiness is stale.
...
| integration config, provider secrets, dashboard settings, active provider config, exact ErrNotFound, env fallback, legacy bootstrap, malformed settings, decrypt failure, stored secret empty, fail closed | `backend/modules/integration_config`, `/dashboard/integrations`, authenticated integration/status APIs | runtime resolver and repository scanner first; stored row owns enabled/settings/secrets, while deployment docs describe only the exact-missing legacy bootstrap path |
| public catalog, published slug, public services, landing page, staff privacy, public readiness, stale published page, call to request, authority-aware catalog, publish fence | `backend/modules/public_catalog`, `backend/modules/salon/repository.go`, `landing/app/s/[slug]/page.tsx` | Settings UI public catalog card, scheduling authority/version |
