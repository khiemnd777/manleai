...
| Normalized Platform tenant control plane | `docs/platform-tenant-api-v2.md`, `backend/modules/ai_runtime_control/*`, Platform handlers in `integration_config`, `pos_square`, `scheduling_manleai_calendar`, `scheduling_authority_switch`, `notification_delivery`, `operations_health`, `tenant_runtime`, `access`, `config_transfer`, plus `frontend/features/platform/*` and `frontend/lib/api/*` | Canonical `/api/v2/platform/tenants/:tenant_id/*` resources are grouped by Business, AI Receptionist, Integrations, Scheduling, Operations, Access, and History. Responses use `{data,meta}`; active frontend tenant-detail consumers use v2. `/technical` is redirect/route compatibility only. Platform Admin acts directly as the actual actor; Ops retains exact delegated grants. AI runtime is authority-neutral and no longer owned by Square. Legacy Platform routes remain additive compatibility adapters over the same domain services/repositories, with no dual write | Platform v2 route/auth tests, frontend evidence tests, V66/V70/V89 safety tests, AI-runtime service/routes tests |
...
| `/platform/tenants/[tenantId]/scheduling`, `/scheduling/calendar` | `frontend/features/platform/platform-scheduling-authority-control.tsx`, `platform-scheduling-settings.tsx`, `platform-internal-calendar-settings.tsx`, `frontend/lib/scheduling/calendar-setup.ts` | One visible authority change command backed by version/readiness/atomicity checks and immutable internal run evidence; Platform Admin needs no Tenant approval. The child setup route owns the cross-object production checklist, groups granular blockers by canonical entity name, deep-links into Staff/Service parent edit flows, and reuses one aggregate load. `/technical` redirects to Integrations for compatibility. |
