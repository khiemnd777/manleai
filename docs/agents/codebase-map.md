...
| Integration config | `backend/modules/integration_config/*` | Authenticated salon-scoped Square/Twilio/OpenAI runtime settings and encrypted secrets; Twilio owns immutable integration-row route UUID, active unique `voice_inbound_number`, explicit routing enablement, masked Account SID hint, computed tenant-bound URLs, provider-only UUID locator, exact tenant reload under RLS, and typed number-conflict handling; Square-only exact-`ErrNotFound` legacy bootstrap resolution; database-only Twilio/OpenAI runtime resolution with no environment inheritance; fail-closed repository/settings/decryption handling; Platform persisted-only reads and secret redaction; owner-SMS settings; strict messaging/audio-token resolvers; and fail-closed OpenAI strict resolution | `backend/modules/integration_config/routes_test.go`, `backend/modules/integration_config/service_test.go`, `backend/modules/integration_config/repository_test.go`, `backend/migrations/v83_*_test.go`, caller propagation tests in Voice and Square adapters |
| Configuration transfer | `backend/modules/config_transfer/*`, `backend/migrations/V77__platform_configuration_transfer.sql`, `frontend/app/platform/tenants/[tenantId]/transfer/page.tsx`, `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/lib/api/platform-configuration-transfer.ts`, `docs/lotus-investor-demo-consultation-pack-v8.json` | Mounted Platform-only tenant-to-tenant/schema-v9 JSON preview/apply/export/run history; exact authorization/fences and atomic audit; active provider, authority, secrets, connections, provider hours, operational history, and Twilio route UUID/inbound number/public callback base/routing enablement/computed URLs/live evidence are excluded and preserved at the destination. Twilio `voice_transport` is the only portable Voice field. Full/runtime/provider v7 and v1-v6 stay rejected; earlier Tenant/onboarding code remains unmounted compatibility. | `backend/modules/config_transfer/*_test.go`, `backend/migrations/v77_safety_test.go`, `frontend/lib/api/platform-configuration-transfer.test.ts` |
...
| Database/migrations | `backend/internal/database/database.go`, `backend/internal/database/migrate.go`, `backend/migrations/*.sql` | Schema drift, startup migrator, migration checksums, version ordering; V44-V82 retain the documented Owner-first, scheduling, access, RLS, and answer-context contracts; V83 adds the expand-only Twilio Voice E.164/routing constraints, global active-number uniqueness, provider-only route-to-tenant locator, and verified-route event/fingerprint indexes without removing shared ingress. |
...
| `/platform/tenants/[tenantId]/technical` | `frontend/features/platform/technical-integration-settings.tsx`, shared `frontend/features/integrations/square-integration.tsx` provider panel, `frontend/lib/api/integration-config-contract.ts`, `technical-scheduling-settings.tsx` | Platform-only Square/Twilio/OpenAI and scheduling controls; the Twilio child shows editable inbound E.164/Account SID/Auth Token/public base/enablement, immutable route ID, computed read-only Voice URLs, blockers, and separate routing-configured versus live-verified evidence. No standalone routing page. |
