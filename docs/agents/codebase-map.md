...
| `/platform/tenants/[tenantId]/business`, `/staff`, `/customers` | `platform-business-workspace.tsx` plus shared Business editors | Profile/hours/public page, Staff, and Customers are separate parent-object workflows over normalized Business v2 resources with actual-actor audit. |
...
| `/platform/tenants/[tenantId]/scheduling`, `/scheduling/calendar` | `frontend/features/platform/platform-scheduling-authority-control.tsx`, `platform-scheduling-settings.tsx`, `platform-internal-calendar-settings.tsx` | One visible authority change command backed by version/readiness/atomicity checks and immutable internal run evidence; Platform Admin needs no Tenant approval. Internal-calendar setup is a separate child route. `/technical` redirects to Integrations for compatibility. |
