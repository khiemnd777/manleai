...
4. Route authority-sensitive scheduling through
   ...
   readiness. Authority switching is an explicit Platform-actor command backed
   by internal preview/commit evidence; it never follows from integration state
   and Platform Admin does not require Tenant approval. Party operations are
   all-or-none, and
...
7. For provider setup or active-runtime diagnosis, treat Platform-managed
   ...
   `GET /api/v2/platform/tenants/:tenant_id/integrations`, the
...
- Normalized Platform tenant v2 contract and IA:
...
- Protected Tenant login-email/password recovery and first Platform Admin bootstrap:
  `backend/cmd/platform-access/main.go`.
...
- Release env/edge cutover contract: the workflow stages candidate
  ...
  markers.
- Production domain contract: `deploy/project.env.example`,
  `deploy/validate-production-domain-contract.sh`,
  `deploy/manleai.caddy.template`, and
  `deploy/manleai.edge-manifest.template`. `ADMIN_DOMAIN` owns the shared
  Platform/Tenant frontend plus primary API; `MARKETING_DOMAIN` owns public
  marketing plus the bounded legacy callback proxy; `LANDING_DOMAIN` owns
  canonical salon pages; `POS_DOMAIN` owns the standalone POS calendar.
...
- Existing-volume production runtime-role bootstrap:
  `.github/workflows/production-runtime-role-bootstrap.yml` invokes
  `deploy/postgres-init-runtime-role.sh` only through the protected
  `production` environment. It requires an exact manual confirmation and
  bounded approval reference, validates the owner/runtime URL split, targets
  exactly one healthy ManleAI PostgreSQL container, reconciles the configured
...
- Emergency production demo identity preparation:
  `.github/workflows/production-demo-identity-bootstrap.yml` invokes
  `deploy/production-demo-identity-bootstrap.sh` only through the protected
  `production` environment. It accepts only the approved one-active-Tenant,
  no-Platform-Admin aggregate state or its exact completed replay state; uses
  `backend/cmd/platform-access` to replace the same Tenant owner's login email
  without changing the user ID, salon ownership, password, memberships, or
  tenant data; revokes that Tenant's refresh tokens; bootstraps one separate
  Platform identity with the freed email and one active Platform Admin role;
  verifies exact aggregate postconditions; and removes temporary credential
  inputs from the runner and VPS.
- Production Platform Admin credential recovery:
  `.github/workflows/production-platform-admin-credential-recovery.yml` invokes
  `deploy/production-platform-admin-credential-recovery.sh` through the
  protected `production` environment. `backend/modules/access/platform_admin_recovery.go`
  owns the serializable exact-one-active-Admin invariant, password-only update,
  assignment-version fence, refresh-token revocation, replay-safe immutable
  audit, and credential-free audit payload. The workflow hands the login result
  back only as a one-day RSA-OAEP-SHA256/AES-256-GCM encrypted artifact and
  removes runner/VPS plaintext in `always()` cleanup.
- Production Tenant Owner credential recovery:
  `.github/workflows/production-tenant-owner-credential-recovery.yml` invokes
  `deploy/production-tenant-owner-credential-recovery.sh` through the protected
  `production` environment. `backend/modules/access/tenant_password_recovery.go`
  owns the serializable exact-salon/email/active-Tenant-owner fence,
  password-only update, refresh-token revocation, replay-safe immutable audit,
  and credential-free result/audit payload. The workflow enforces the exact
  two-account aggregate before and after and removes runner/VPS plaintext in
  `always()` cleanup.
...
- Local orchestration: root `make restart` delegates to
  ...
  builds the stack, validates the rendered server-only landing API origin
  against the Compose service inventory through
  `deploy/local-environment-contract.sh`, passes the required browser API
  origin into the landing image build and runtime, runs
  `deploy/postgres-migration-preflight.sh`, resets only an incompatible
  Compose-owned local sample PostgreSQL volume, invokes the guarded fixture
  runner, verifies landing-to-API `/healthz` connectivity and `sample_test` through
  `deploy/postgres-data-profile-guard.sh`, and waits for all services. Local
  Compose fixes `DEPLOYMENT_ENV=local`; `make restart-prod-sim` selects
  `APP_ENV=production` without leaving local database/Redis/URL/volume
  boundaries. `deploy/local-backend-integration-test.sh` creates disposable
  release-gate databases and verifies application user/salon counts remain
  unchanged.
...
| Marketing registration and Tenant provisioning | `backend/modules/tenant_registration/*`, `backend/modules/tenant_provisioning/*`, `backend/migrations/V85__tenant_registration_and_provisioning.sql`, `frontend/features/platform/tenant-registration-*`, `frontend/lib/api/tenant-registration.ts`, `frontend/lib/api/tenant-registration-routes.ts`, `frontend/lib/api/tenant-registration-types.ts`, `frontend/features/auth/accept-owner-invitation-form.tsx` | Public consented pre-tenant intake with exact submission replay, duplicate evidence, rate limiting, immutable review audit/notes, Admin/Ops queue, versioned state/assignment actions, Admin-only active Tenant-identity search and atomic provisioning, fixed initial `owner_manual`, one-time hashed Owner invitations, and 180-day terminal PII redaction; intake never configures providers or enables booking | module/service and PostgreSQL repository integration tests, runtime-role RLS integration test, V85 safety test, rate-limit tests, frontend contract/typecheck/build, and landing contract tests |
| SaaS access control | `backend/modules/access/*`, `backend/modules/pos/repository.go`, `backend/modules/training/repository.go`, `backend/internal/middleware/auth.go`, `backend/cmd/platform-access/main.go`, `backend/migrations/V64__saas_access_control_foundation.sql`, `backend/migrations/V74__principal_scope_isolation.sql`, `backend/migrations/V75__owner_authorized_platform_support.sql`, `backend/migrations/V76__platform_admin_authority_and_ops_delegation.sql`, `backend/migrations/V89__platform_authority_actual_actor.sql`, `frontend/features/platform/access-console.tsx`, `frontend/features/platform/tenant-access-console.tsx`, `frontend/features/platform/access-ui.tsx`, `frontend/lib/api/access-contract.ts`, `frontend/lib/api/access.ts` | Tenant memberships with exact-route active-member enforcement; Owner suspension/reactivation while preserving `salons.owner_user_id`; immutable Tenant/Platform identity realms; direct create/edit/password/role/status management for Platform identities with bcrypt, session revocation, action replay, version fences, and last-admin governance; Platform Admin direct role-capability authority across all salon Platform tabs without salon assignment, temporary authorization, PII grant, or Tenant approval; exact-salon baseline and Admin-granted time-bounded feature/PII access for Platform Ops; POS/Training/domain repositories and V89 authority-switch persistence use the actual actor through `app_actor_feature_access`; raw support helpers remain internal Ops sub-rules; immediate revoke/expiry/role-or-assignment invalidation; actual-actor immutable audit | `backend/modules/access/*_test.go`, `backend/modules/pos/repository_actor_feature_access_integration_test.go`, `frontend/lib/api/access.test.ts`, `backend/internal/middleware/auth_test.go`, `backend/modules/auth/repository_integration_test.go`, `backend/migrations/v64_safety_test.go`, `backend/migrations/v74_safety_test.go`, `backend/migrations/v75_safety_test.go`, `backend/migrations/v76_safety_test.go`, `backend/migrations/v89_safety_test.go` |
| SaaS shared Business management | `backend/modules/business/*`, `backend/migrations/V65__saas_business_management.sql`, `backend/modules/salon/repository.go` public-catalog readiness helper | Tenant `/api/salons/:id/business/*` remains stable; canonical Platform v2 owns the salon collection/context, `/business/profile`, `/business/hours`, `/business/public-page`, `/staff`, and `/customers`. Rich `/services` is owned by `modules/pos`, avoiding competing route/schema owners. Legacy Platform Business routes are compatibility adapters over the same services/repositories. Platform staff contacts remain omitted and unwritable; provider-linked operational fields and external hours remain read-only; exact action-key replay, versions, actual-actor audit, scheduling fences, safe audit allowlists, and provider/legacy version triggers remain intact | `backend/modules/business/*_test.go`, `backend/migrations/v65_safety_test.go`, `backend/modules/salon/public_catalog_integration_test.go` |
| Normalized Platform tenant control plane | `docs/platform-tenant-api-v2.md`, `backend/modules/ai_runtime_control/*`, `backend/modules/scheduling_behavior/*`, Platform handlers in `integration_config`, `pos_square`, `scheduling_manleai_calendar`, `scheduling_authority_switch`, `notification_delivery`, `operations_health`, `tenant_runtime`, `access`, `config_transfer`, plus `frontend/features/platform/*` and `frontend/lib/api/*` | Canonical `/api/v2/platform/tenants/:tenant_id/*` resources are grouped by Business, AI Receptionist, Integrations, Scheduling, Operations, Access, and History. Responses use `{data,meta}`; active frontend tenant-detail consumers use v2. `/technical` is redirect/route compatibility only. Platform Admin acts directly as the actual actor; Ops retains exact delegated grants. AI runtime is authority-neutral and no longer owned by Square. Scheduling behavior exposes persisted authority plus booking mode and manages booking mode through a separate versioned/idempotent/audited command without changing authority. Legacy Platform routes remain additive compatibility adapters over the same domain services/repositories, with no dual write | Platform v2 route/auth tests, scheduling-behavior service/repository/RBAC tests, frontend evidence tests, V66/V70/V89 safety tests, AI-runtime service/routes tests |
| SaaS runtime isolation and fairness | `backend/internal/database/context_connector.go`, `backend/internal/database/runtime_role.go`, `backend/internal/databasecontext/*`, `backend/modules/tenant_runtime/*`, `backend/modules/conversation/repository.go`, `backend/modules/conversation/scheduling_result_evidence.go`, `backend/modules/salon/repository.go`, `backend/modules/booking/repository.go`, `backend/modules/scheduling/repository.go`, `backend/modules/scheduling_owner_manual/repository.go`, `backend/modules/scheduling_manleai_calendar/*repository*.go`, `backend/modules/pos_square/service.go`, `backend/migrations/V67__tenant_runtime_membership_boundary.sql`, `V68__saas_row_level_security.sql`, `V69__tenant_runtime_quotas_usage.sql`, `V71__public_catalog_projection_boundary.sql`, `V72__platform_pii_scope_enforcement.sql`, `V75__owner_authorized_platform_support.sql`, `V76__platform_admin_authority_and_ops_delegation.sql`, `V78__system_tenant_context_expand.sql`, `V79__system_tenant_contract_preparation.sql`, `V80__strict_system_tenant_rls_contract.sql`, `V90__worker_claim_atomicity.sql`, `V91__manleai_calendar_actual_actor.sql` | Request actor/system scope is applied per DB operation and reset before connection reuse; production runtime role must be non-owner/non-superuser/non-BYPASSRLS; tenant row isolation and active-membership runtime gate; public safe projection; V76 RLS gives Platform Admin direct role-capability access including PII while Platform Ops still requires exact assignment, temporary feature authorization, and exact active PII grant; V78 adds `app.system_salon_id` and provider locators; V79 moves global worker claim/recovery/cleanup/retention discovery behind bounded worker-only functions, binds returned items to one salon, and enforces composite call-child salon/session keys; V80 requires exact system salon matching for every ordinary provider/worker base-table policy and audits the final policy catalog; V90 keeps the same worker APIs/fairness while rechecking current eligibility at candidate lock and final update time for POS sync, Square webhook, owner/customer notification, and OpenAI verification claims; V91 preserves actual Calendar actors and aligns DB audit/activation/exception guards with Tenant membership or Platform `technical.write`. Live conversation and all three scheduling-authority repository paths use the same exact system-salon predicate independently of Tenant Owner membership, while interactive Actor paths retain membership/capability authorization; unbound and cross-tenant system contexts fail closed. Calls scheduling evidence remains select-only and durably call-linked; database-owned tenant quotas/usage plus fair single-winner worker claim caps | `backend/internal/database/*_test.go` including revoked-Owner provider read/write/replay coverage, `backend/modules/tenant_runtime/*_test.go`, migration safety tests through `v91_safety_test.go`, `backend/migrations/v79_integration_test.go`, `backend/migrations/v80_integration_test.go`, `backend/migrations/v90_integration_test.go`, `backend/modules/scheduling_manleai_calendar/platform_actor_integration_test.go` |
| Runtime environment and sample-test data profile | `backend/internal/config/config.go`, `backend/cmd/sample-data/main.go`, `backend/sampledata/*`, `backend/sampledata/migrations/*.sql`, `backend/migrations/V73__sample_data_classification.sql`, `backend/migrations/V74__principal_scope_isolation.sql`, `docker-compose.yml`, `docker-compose.prod.yml`, `deploy/local-restart.sh`, `deploy/local-environment-contract*.sh`, `deploy/local-backend-integration-test.sh`, `deploy/postgres-*-preflight.sh`, `deploy/postgres-data-profile-guard.sh`, `landing/lib/http-origin.ts`, `landing/lib/api.ts`, `landing/lib/config.ts`, `.github/workflows/ci-cd.yml`, `backend/ent/schema/user.go`, `backend/ent/schema/salon.go`, `frontend/features/platform/tenant-directory.tsx`, `frontend/features/platform/tenant-detail-shell.tsx`, `frontend/features/platform/access-console.tsx`, `frontend/features/platform/tenant-access-console.tsx`, `frontend/features/platform/access-ui.tsx`, `frontend/features/dashboard/dashboard-home.tsx` | Compose owns immutable physical `local|production` deployment selection; behavior inherits it and local alone may simulate production. Production cannot downgrade. Deployment-only security follows the physical target. Compose also owns the server-only landing API origin; local restart validates its rendered hostname against the Compose service inventory and verifies `/healthz`, while browser API calls use the separately required public origin. The landing server resolves its internal origin lazily at request time so image builds remain runtime-independent. Normal migrations add default-`live` classification without fixtures; guarded local/pre-live fixture replay remains sample-only; local integration packages run in disposable isolated databases rather than the application database | config/sampledata/auth tests, `backend/migrations/v73_safety_test.go`, `backend/migrations/v74_safety_test.go`, `deploy/local-environment-contract-test.sh`, `landing/lib/http-origin.test.ts`, `landing/lib/api-config.test.ts`, release-gate shell syntax self-test, local restart/profile verification, isolated local integration gate, `frontend/lib/api/business.test.ts` |
...
| POS provider-neutral layer | `backend/modules/pos/*` | `POSProvider` and scoped scheduling-capability contracts, typed provider-write outcome/phase errors, stable safe error-code-to-message mapping that excludes provider response text, V87 persisted-only connection/config/location/API/scope capability resolution and immutable idempotent evidence, V88 explicit unconfigured-active-provider state, cross-tenant provider/merchant/location uniqueness, reconnect invalidation, and versioned/idempotent initial-provider activation, derived service/staff field authority and provider-managed write guards, current-authority-aware transactional canonical `ai_bookable` ownership in `Repository.UpdateServiceAIBookable`/`UpdateStaffAIBookable` using the shared booking/calendar advisory lock plus scheduling-authority/version and active-provider link evidence, atomic service owner-controls, monotonic location/generation-fenced full snapshots, optional appointment listing capability, POS entity links, service/staff/customer catalog, service consultation profile persistence/validation, sync jobs/logs/errors, provider switching, salon categories/aliases, V58 cross-table active alias ownership locking/validation, V63 historical provider-diagnostic redaction, V75/V76 Platform Services/Calls catalog routes with direct Admin or delegated Ops authorization and fail-closed action audit, actual-actor repository authorization without Owner-ID substitution, and materialization from the active database-owned nail taxonomy release | `backend/modules/pos/service_test.go`, `backend/modules/pos/handler_test.go`, `backend/modules/pos/platform_support_routes_test.go`, `backend/modules/pos/types_test.go`, `backend/modules/pos/sync_processor_test.go`, `backend/modules/pos/repository_integration_test.go`, `backend/modules/pos/initial_provider_activation_integration_test.go`, `backend/modules/pos/repository_scheduling_capability_test.go`, `backend/modules/pos/repository_scheduling_capability_integration_test.go`, `backend/modules/pos/repository_taxonomy_integration_test.go`, `backend/modules/pos/repository_alias_ownership_integration_test.go`, `backend/migrations/v58_safety_test.go`, `backend/migrations/v63_safety_test.go`, `backend/migrations/v75_safety_test.go`, `backend/migrations/v76_safety_test.go`, `backend/migrations/v87_safety_test.go`, `backend/migrations/v88_safety_test.go` |
| Square adapter | `backend/modules/pos_square/*` | Mandatory explicit salon context plus a constructor-required database-backed config resolver for every OAuth/API path; no process-global Square runtime configuration; buyer-level OAuth (`APPOINTMENTS_WRITE`, never new `APPOINTMENTS_ALL_WRITE`), locations, atomic location-scoped generation-fenced sync with fail-closed freshness, Catalog `available_for_booking`/duration eligibility, active Team plus bookable Booking Profile intersection, Square payloads, salon-local availability ranges, provider-neutral `external_provider` target readiness with operation-specific V87 safety gates: current buyer-write concrete-staff single create only, seller-write/reschedule/party/resource fail-closed; Platform Integrations connection/sync/activation/safety verification with actual-actor audit and no provider call/manual override; authority-neutral AI runtime is owned separately by `modules/ai_runtime_control`; dashboard test create/cancel writes through the scheduling facade with provider-free history replay and read-only create-lineage authorization for persisted external retries, post-switch external cleanup, sanitized gate errors, stable error classification without returning or persisting Square response details, provider-fenced booking-list pagination with cross-location rejection, token refresh, provider write outcome/error mapping, signed booking-webhook ingestion with root/nested location consistency, exact tenant routing across recoverable connection states, claim-token-fenced event processing and provider/connection-scoped scheduled calendar repair for historical external mirrors, plus normalized Operations provider-event list/detail/requeue with no raw/provider/PII exposure | `backend/modules/pos_square/*_test.go`, `backend/migrations/v60_safety_test.go`, `backend/migrations/v63_safety_test.go`, `backend/migrations/v87_safety_test.go`, `backend/migrations/v88_safety_test.go` |
...
| Conversation runtime | `backend/modules/conversation/*`, `backend/migrations/V81__conversation_answer_context_resource_versions.sql`, `backend/migrations/V82__conversation_answer_context_collection_cascade_safety.sql` | Simulator/phone session state; neutral scheduling availability/action use plus provider-free reschedule-candidate history; Phase 5H `booking_mode` matrix with mode/selected-authority review fence, verified-slot-then-request pending flows for internal/external authorities, owner-manual request-only pending flow, disabled pre-planner zero scheduling dispatch, structured party/time fidelity, and no pending confirmation wording; aggregate ManleAI create proof and Phase 4C lifecycle state in `service_internal_lifecycle.go`, including exact target/version, reviewed reschedule quote, cancel reason, explicit state-scoped authorization, target-origin execution after current-authority change, exact historical replay, stale refresh/re-offer, cutoff handoff, durable status/version/active-child validation, and provider-evidence rejection; persisted catalog-backed fuzzy candidate confirmation in `service_fuzzy_confirmation.go`, with exact/alias behavior unchanged and no draft/availability/booking mutation before explicit confirmation; external party/catalog/provider behavior preserved; safe handoff/no false confirmation; per-session serialization, state-revision CAS, event-stable reply replay, database-fenced answer context with persisted common service/staff/alias/category/profile/knowledge collection versions and authority-relevant evidence only, including a lightweight per-turn ManleAI config/activation fence and authoritative aggregate readiness only on cache miss; low-cardinality answer-context timing distinguishes hit, TTL/fence miss, readiness/concurrent retry, fail-closed refresh, load error, and retry exhaustion without IDs, transcript, or raw errors; V82 preserves explicit service/staff mutation bumps while preventing a salon cascade delete from recreating a resource-version child after its parent is gone; Turn Kernel, service understanding, party planning, consultation/safety, handoff, transcript metadata, and retention; valid legacy non-object transcript metadata and non-array party guest JSON are normalized out of the detail projection with safe `detail_warnings`, while database/read failures retain a typed transcript, handoff, party-request, or scheduling-evidence stage and never serialize raw errors; V62 party-request hydration and session redaction are fenced by the same salon/session parent key; Platform `platform_handler.go` reuses the same service for direct Admin or currently Admin-granted Ops Calls read/manage/simulate/redact actions, preserves that Platform identity through runtime/scheduling repositories, requires Calls PII for Ops, and records actual-actor audit rather than substituting the salon Owner | `backend/modules/conversation/*_test.go`, `backend/modules/conversation/booking_mode_matrix_test.go`, `backend/modules/conversation/owner_manual_scheduling_test.go`, `backend/modules/conversation/manleai_calendar_scheduling_test.go`, `backend/modules/conversation/manleai_calendar_party_scheduling_test.go`, `backend/modules/conversation/manleai_calendar_lifecycle_conversation_test.go`, `backend/modules/conversation/fuzzy_service_confirmation_test.go`, `backend/modules/conversation/answer_context_test.go`, `backend/modules/conversation/answer_context_performance_test.go`, `backend/modules/conversation/answer_context_performance_integration_test.go`, `backend/modules/conversation/repository_answer_context_fence_integration_test.go`, `backend/modules/conversation/party_tenant_integrity_integration_test.go`, `backend/modules/voice_twilio/phone_booking_flow_test.go`, `backend/migrations/v76_safety_test.go`, `backend/migrations/v81_safety_test.go`, `backend/migrations/v82_safety_test.go` |
| Conversation AnswerContext performance/freshness verification | `backend/modules/conversation/answer_context_performance_test.go`, `backend/modules/conversation/answer_context_performance_integration_test.go`, `docs/operations/conversation-answer-context-performance.md` | Dedicated P2 evidence for exact per-authority PostgreSQL round trips, stable ManleAI cache-hit aggregate bypass, TTL/fence/readiness/concurrent retry classification, separate process-local replica freshness, tenant isolation, bounded retry exhaustion, synthetic bounded `EXPLAIN (ANALYZE, BUFFERS)`, and an explicitly in-memory Go cache benchmark. It does not reuse the scheduling load harness or claim production capacity. | Conversation/voice unit tests, isolated PostgreSQL integration tests, benchmark, backend release gate |
| Training | `backend/modules/training/*` | Knowledge items, owner corrections, correction apply/dismiss, service alias application, V58 salon/normalized-alias ownership locking with typed cross-namespace validation, and read-only training evaluation whose owner-scoped selected authority comes from `scheduling.Service.CurrentSchedulingAuthority`; Tenant routes require active exact membership and capability; Platform routes allow direct Admin or currently Admin-granted Ops `training.read|write`, with Calls PII required for transcript-linked Ops corrections; repository predicates use canonical `app_actor_feature_access`; service-alias application writes canonical structured `service_aliases`, never knowledge text; evaluation returns authority-native confirmation requirement/guardrail fields and never dispatches a scheduling executor/provider | `backend/modules/training/service_test.go`, `backend/modules/pos/repository_alias_ownership_integration_test.go`, `backend/modules/pos/repository_actor_feature_access_integration_test.go`, `backend/migrations/v58_safety_test.go`, `backend/migrations/v76_safety_test.go` |
| Voice provider-neutral | `backend/modules/voice/*` | Owner-scoped, authority/version-fenced voice status with separate `phone_answering`, `request_capture`, and `automated_booking` dimensions; normalized Platform `/calls/readiness` remains a static Voice-owned resource because Conversation v2 session routes require GUID identifiers, with a composed production-order regression test in `voice/routes_test.go`; Twilio phone answering now requires the stored tenant-bound route/inbound E.164/Account SID/Auth Token/public HTTPS base rather than `salons.phone`; `twilio_route.go` owns the server-only resolved/verified route proof, immediate provider tenant binding, route/number/AccountSid/CallSid agreement, no-side-effect rejection, and current-fingerprint live evidence; `technical.go` owns Platform-safe configured-versus-live routing status; target readiness remains owned by the selected scheduling executor; also owns semantic-contract verification, bounded read-only semantic evaluation, guarded model-reply conversion, safe provider diagnostics, STT/LLM/TTS/realtime and streaming-speech interfaces, speech turns, and public audio delivery through metadata-first bounded HMAC capabilities | `backend/modules/voice/*_test.go`, `backend/modules/voice_twilio/handler_test.go`, `backend/modules/conversation/repository_concurrency_integration_test.go`, `backend/migrations/v83_*_test.go` |
| Conversation evaluation | `backend/internal/conversationeval/*`, `backend/cmd/conversation-eval*/*`, `docs/operations/conversation-semantic-quality.md` | Three explicitly different evidence sets: (1) the schema-v2 1,000-scenario single-turn semantic contract corpus with a 100-record deterministic source/state audit, which is not a 100-conversation suite; (2) the directly authored 50-execution/45-situation single-turn direct-model pilot, whose P3 informational balance covers distinct hours, staff, and policy meanings plus staff preference; and (3) the independent 100-journey real-salon multi-turn corpus and `conversation-eval-real` runner. The real-salon suite has exact 15/15/10/20/10/10/10/5/5 family coverage, three-to-twelve retained caller turns, at least three distinct catalog fixtures, zero generated/paraphrase journeys, complete session/state/tool evidence including turn route, interpreter outcome, answer source/reason, router intent, reply source/policy, deterministic execution of all 100 through the production Conversation Service, and honest structural/runtime statuses that never claim a model pass. P3 classifies failures by data/cache/model/routing/copy/style/voice/scheduling/fixture owner before any prompt change. Exactly ten P3-balanced live journeys are separately marked as canaries; their database-configured OpenAI run checkpoints every request, stops on the first journey failure, caps all recognition/reply/consultation/review calls at 60, reviews two batches of five complete interleaved transcripts, and passes only when all ten model executions and reviews pass. All direct and real-salon evaluators use isolated in-memory state and side-effect-blocking booking adapters; they cannot persist production conversations, mutate appointments, call POS, or manufacture booking confirmation evidence. Existing authenticated semantic runtime canaries remain capped at 12. | `backend/internal/conversationeval/*_test.go`, `backend/cmd/conversation-eval/*_test.go` |
| Scheduling load/concurrency verification | `backend/internal/schedulingload/*`, `backend/cmd/scheduling-load-harness/main.go` | Bounded synthetic `owner_manual`, `manleai_calendar`, V86/V87 external Atomic Slot Commit, and authority-switch replay/CAS/resource-conflict verification against an explicitly attested isolated PostgreSQL database; each external replica owns an independent pool/repository/service and uses only a capability-aware fake provider; one-slot/overlap/replay/failure/unknown/reschedule/cancel/cross-tenant workloads; exact DB name/user/prefix and release-migration checksum guards; no destructive cleanup; unique run/tenant evidence; zero real POS/Twilio/OpenAI calls; PII-free schema-v2 latency/throughput/conflict/invariant/per-replica-pool report; pass requires exact expected conflicts/dispatches, retained unknown claims, and zero safety, tenant, idempotency, duplicate, orphan, loser-dispatch, unexpected-release, provider-evidence, and provider-call violations. This harness is not production capacity proof without an approved witnessed representative run. | `backend/internal/schedulingload/*_test.go`, opt-in `TestRunAgainstFreshIsolatedPostgres` |
| Twilio Voice adapter | `backend/modules/voice_twilio/*`, `backend/internal/twiliowebhook/*`, `backend/internal/twiliovoice/*` | Tenant-bound `/:route_id` incoming/turn/recording/stream/status/fallback ingress; exact stored callback URL plus all-form verification through Twilio's official request validator; route proof propagation with no second phone-based tenant lookup; durable CallSid binding for callbacks whose vendor payload omits `To`; Media Streams token/session fence; allowlisted audit fields instead of provider bodies; TwiML, recording mode, caller-input gate, typed reply scheduling, stale-generation suppression, recovery first-byte budget, terminal latch, and bounded diagnostics. Shared routes remain expand-release rollback compatibility until the contract release. | `backend/internal/twiliowebhook/*_test.go`, `backend/internal/twiliovoice/*_test.go`, `backend/modules/voice_twilio/*_test.go`, `backend/migrations/v83_*_test.go` |
| OpenAI voice adapter | `backend/internal/openairuntime/*`, `backend/modules/voice_openai/*` | Strict tenant resolver required in production; blank/cross-tenant requests fail before network; server-owned `openai_public` HTTP/WSS destination with redirects/env proxy/unsafe dial denied; config identity uses tenant/config/credential-revision/destination/schema fences without plaintext key material. Owns OpenAI STT, strict full/guidance structured-turn schemas with catalog-bound normalization, guarded operational/consultation reply, evaluation token observation, whole/streaming Speech-to-PCMU, Realtime input/noise admission, bounded provider diagnostics, per-schema circuits, and explicit capability probes. | `backend/internal/openairuntime/*_test.go`, `backend/modules/voice_openai/*_test.go` |
| Integration config | `backend/modules/integration_config/*` | Authenticated salon-scoped Square/Twilio/OpenAI runtime settings and encrypted secrets. Square, Twilio, and OpenAI runtime resolvers all require their exact enabled `salon_integration_configs` row and fail closed on missing/malformed/decryption failures; none inherit process configuration. Twilio owns immutable integration-row route UUID, active unique inbound E.164, routing enablement, masked Account SID hint, computed tenant URLs, provider-only UUID locator, exact-tenant RLS reload, number-conflict handling, owner-SMS settings, and strict messaging/audio resolvers. OpenAI owns the fixed destination profile, purpose-separated unique credential HMAC identity, monotonic credential revision, shared runtime validator, typed duplicate-key conflict, and strict `ResolveOpenAIRuntimeConfig`. | integration-config tests, V83/V84/V88 migration tests, Voice/Square caller propagation tests |
| OpenAI runtime verification | `backend/modules/openai_runtime_verification/*`, `backend/migrations/V84__openai_tenant_bound_runtime.sql`, `docs/operations/openai-tenant-runtime.md` | Platform `technical.read|write` GET/POST; HTTP POST queues only; durable config/credential/destination/contract fences; per-capability results and immutable safe events; bounded unbound worker claim followed by exact returned-tenant binding; GET has zero provider calls; fake transports in CI | V84 safety tests, runtime/adapter tests, worker registration and Operations Health |
| Configuration transfer | `backend/modules/config_transfer/*`, `backend/migrations/V77__platform_configuration_transfer.sql`, `frontend/app/platform/tenants/[tenantId]/transfer/page.tsx`, `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/lib/api/platform-configuration-transfer.ts` | Mounted Platform-only tenant-to-tenant/schema-v10 JSON preview/apply/export/run history with schema-v9/v8 input compatibility and guarded content-only v7 adaptation; exact authorization/fences and atomic audit. Active provider, authority, secrets, connections, provider hours, operational history, Twilio route/inbound/public callback/routing/live evidence, and OpenAI enabled state/destination/credential revision/HMAC/key/verification are excluded and preserved at the destination. Twilio transport and OpenAI model/voice settings are portable; a new OpenAI target row stays disabled. | config-transfer backend/frontend tests, V77/V84 migration tests |
...
| Public catalog API | `backend/modules/public_catalog/*`, `backend/migrations/V71__public_catalog_projection_boundary.sql`, `landing/app/s/[slug]/*`, `landing/lib/api.ts` | `/api/public/salons/:slug` and canonical `/s/[slug]`; repository consumes only the database-owned authority-aware JSON projection; public scope sees zero base-table rows; owner-manual canonical services, current activated internal catalog/local hours, or current synced external projections; stale publication fails closed; no provider identifiers, owner IDs, staff contacts, secrets, diagnostics, or confirmation action | public catalog/service/salon integration tests, V71 safety and RLS integration tests |
...
| Database/migrations | `backend/internal/database/database.go`, `backend/internal/database/migrate.go`, `backend/migrations/*.sql` | Schema drift, startup migrator, migration checksums, and version ordering; V44-V82 retain documented Owner-first, scheduling, access, RLS, and answer-context contracts; V83 adds expand-only Twilio tenant routes/E.164 uniqueness/provider locator; V84 adds OpenAI credential identity/destination, Configuration Transfer v10 contract extension, durable capability verification, exact-tenant RLS, and bounded worker claim; V86 owns external Atomic Slot Commit claims/intervals/events; V87 owns monotonic Square connection capability fences plus immutable scoped review evidence/actions; V88 normalizes and uniquely owns complete provider/merchant/location identities across tenants, permits an explicit blank active provider, and seeds the initial-activation version fence. |
| Runtime database isolation | `backend/internal/database/context_connector.go`, `backend/internal/database/runtime_role.go`, `backend/internal/databasecontext/*`, `backend/migrations/V67*` through `V91*`, `deploy/postgres-init-runtime-role.sh`, `.github/workflows/production-runtime-role-bootstrap.yml` | V66 technical ledgers; V67 active membership; V68 RLS; V69 quotas/usage; V70 AI runtime technical audit; V71 public safe projection and zero public base-row visibility; V72 exact Platform PII scopes; V75/V76 feature and Platform authority policies; V78 expand-only system tenant context plus provider ID locators; V79 bounded worker discovery plus call-child composite tenant keys; V80 exact provider/worker system-tenant base-policy matching and policy-catalog audit; V90 backward-compatible single-winner eligibility rechecks for every affected ranked worker claim; V91 actual-actor Calendar technical-write alignment. API migrates with `MIGRATION_DATABASE_URL`, then serves through the separate verified `DATABASE_URL` runtime role; worker receives runtime credentials only. The protected manual bootstrap reconciles the restricted login on an existing production volume without migration or application-row writes. V80 production cutover requires every running image to be V79-aware before migration. | database context/runtime/RLS integration tests, bootstrap workflow/script validation, and V66-V91 safety tests |
...
- Canonical services/categories/aliases own guidance identity. Authority-
  ...
  staff-only/pooled policies and `EvaluateReadiness` capabilities. The answer-
  context freshness fence is owned by `answer_context.go`/`repository.go` and
  V81/V82: common collection versions cover service/staff catalogs, aliases,
...
- Answer-source routing and cached structured context:
  ...
  selects that day's authority-owned periods. `owner_manual` reads owner-
  managed `local_override` rows without a POS or calendar-activation
  dependency; `manleai_calendar` reads `local_override` rows only behind its
  current activated capability/config fence; `external_provider` reads only
  the current active provider/location imported snapshot. Owner-local changes
  invalidate the 45-second conversation cache through the existing
  `business_resource_versions` `business_hours` version; internal config and
  external snapshot changes retain their existing fences. For
  `manleai_calendar`, the every-turn path reads only persisted authority,
  config-version, and activated-version evidence. A stable cache hit does not
  load the aggregate or recompute readiness; a cache miss loads the
  authoritative aggregate, calls `EvaluateReadiness`, and accepts it only when
  those versions match. V81 adds persisted
  `service/collection` and `staff/collection` revisions and composes them with
  the V77 alias/category/consultation/knowledge collection revisions on every
  turn. V82 retains those bumps for explicit service/staff mutations while
  skipping resource-version recreation during parent-salon cascade deletion.
  Provider connection fields are normalized out for `owner_manual` and
  `manleai_calendar`, and local-hours revisions are normalized out where that
  exact value is not the hours owner. Cache misses still double-read the whole
  fence and retry when any consumed source changes. Informational hours never
  prove availability or authorize a scheduling action.
...
| --- | --- | --- |
| `/dashboard/*` | `frontend/app/dashboard/layout.tsx`, `frontend/components/layout/surface-gate.tsx`, `frontend/components/layout/tenant-salon-context.tsx`, `frontend/components/layout/app-shell.tsx`, `frontend/components/layout/powered-by.tsx` | Tenant-only shell; exact active membership selects the salon; Platform-only users are rejected; the shared shell renders fixed KNASOFTWARE attribution after page content and links it to `https://www.knasoftware.com` in a new tab |
...
| `/dashboard/calls` | `frontend/features/dashboard/calls-dashboard.tsx` | Reused full Calls dashboard for sessions, transcripts, detected details, corrections, lifecycle actions, simulator, party review, and authority-native scheduling evidence; a selected-call detail failure stays local to the transcript/detail workflow so readiness and the session log remain usable |
...
| `/platform`, `/platform/tenants` | `frontend/components/layout/platform-app-shell.tsx`, `frontend/components/layout/powered-by.tsx`, `frontend/features/platform/tenant-directory.tsx` | Platform-only shell and normalized v2 tenant directory; the shared shell renders fixed KNASOFTWARE attribution after page content and links it to `https://www.knasoftware.com` in a new tab |
| `/platform/registration-requests`, `/platform/registration-requests/[requestId]` | `frontend/features/platform/tenant-registration-queue.tsx`, `frontend/features/platform/tenant-registration-detail.tsx`, `frontend/features/platform/tenant-registration-contract.ts`, `frontend/lib/api/tenant-registration.ts`, `frontend/lib/api/tenant-registration-routes.ts`, `frontend/lib/api/tenant-registration-types.ts` | Admin/Ops pre-tenant review queue with masked list, authorized detail, filters, assignment, lifecycle, immutable notes/events, verified provisioning draft, Admin-only existing-identity search/provision/invite, optimistic versions, and exact action replay |
...
| `/platform/tenants/[tenantId]/*` shell | `frontend/features/platform/tenant-detail-shell.tsx`, `frontend/lib/api/platform-tenant-context.ts` | Backend-derived capability visibility organized into five workflow groups: Overview, Business, AI Receptionist, Platform Controls, and History. Frontend visibility is a hint; every API call reauthorizes. |
| `/platform/tenants/[tenantId]/overview` | `frontend/app/platform/tenants/[tenantId]/overview/page.tsx`, `frontend/features/platform/platform-business-workspace.tsx` | Read-only salon summary and workflow entry point; does not mix setup, live operations, or audit controls. |
| `/platform/tenants/[tenantId]/business`, `/staff`, `/customers` | `platform-business-workspace.tsx` plus shared Business editors; Platform Staff composes `frontend/features/dashboard/staff-calendar-profile.tsx` | Profile/hours/public page, Staff, and Customers are separate parent-object workflows over normalized Business v2 resources with actual-actor audit. Staff keeps canonical service eligibility in the Business editor and adds Platform-surface internal weekly schedule/time-off controls in the same staff edit flow; calendar writes preserve the persisted eligibility set and use normalized v2 scheduling routes. |
| `/platform/tenants/[tenantId]/services` | shared `frontend/features/dashboard/services-dashboard.tsx` with `surface="platform"` | Rich Services workflow is the sole `/services` owner: provider field authority, categories, category aliases, service aliases, consultation, and service-grain calendar policy. Platform Admin is direct; Ops needs exact Services authorization. Provider setup links to Integrations. |
| `/platform/tenants/[tenantId]/calls` | shared `frontend/features/dashboard/calls-dashboard.tsx` with `surface="platform"` | Calls, party requests, readiness, catalog projections, and corrections use v2 Calls resources; selected-call detail loading/error/retry is isolated from the already-loaded list/readiness state; Platform Admin is direct while Ops retains exact authorization plus Calls PII. |
| `/platform/tenants/[tenantId]/training` | shared `frontend/features/dashboard/training-dashboard.tsx` with `surface="platform"` | Knowledge, corrections, and evaluations use normalized v2 resources; structured service aliases remain owned by Services. |
| `/platform/tenants/[tenantId]/runtime` | `frontend/features/platform/ai-runtime-control.tsx`, `frontend/lib/api/ai-runtime-control.ts` | Authority-neutral AI Receptionist on/off intent. It does not belong to Square and does not change authority, provider configuration, prompt behavior, or operation origin. |
| `/platform/tenants/[tenantId]/integrations` | `frontend/features/platform/platform-integration-settings.tsx`, `frontend/lib/api/integration-config-contract.ts` | Provider credentials, connection, sync, activation, and live verification. Square connection/sync never changes authority; no AI runtime toggle is present. Twilio and OpenAI keep saved configuration distinct from live evidence. |
| `/platform/tenants/[tenantId]/scheduling`, `/scheduling/calendar` | `frontend/features/platform/platform-scheduling-authority-control.tsx`, `platform-scheduling-settings.tsx`, `platform-internal-calendar-settings.tsx`, `frontend/lib/api/scheduling-behavior.ts`, `frontend/lib/scheduling/calendar-setup.ts` | One Scheduling behavior card loads persisted authority plus booking mode, shows backend-owned allowed/effective behavior, and keeps authority switching and booking-mode saving as separate explicit actions. Authority change retains version/readiness/atomicity checks and immutable run evidence; booking mode uses the `ai_receptionist/policy` version, action replay, actual-actor audit, and the same scheduling fence. Preview/latest readiness evidence is not labelled as a committed change. Platform Admin needs no Tenant approval. The child setup route owns the cross-object production checklist, groups granular blockers by canonical entity name, deep-links into Staff/Service parent edit flows, and reuses one aggregate load. `/technical` redirects to Integrations for compatibility. |
| `/platform/tenants/[tenantId]/operations` | `frontend/features/platform/platform-operations-console.tsx`, `tenant-runtime-controls.tsx` | Local workflow tabs for health/queues, runtime limits, Square provider events, and owner notifications over normalized Operations resources. |
| `/platform/tenants/[tenantId]/access` | `frontend/features/platform/tenant-access-console.tsx`, `frontend/features/platform/access-ui.tsx` | Local workflow tabs for salon team, Platform support, temporary Ops, and sensitive data; one normalized aggregate read plus v2 mutations. |
| `/platform/tenants/[tenantId]/transfer` | `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/lib/api/platform-configuration-transfer.ts` | Platform Controls configuration copy with preview/apply/stale/conflict/replay; destination implied by route; no authority/provider credential/secret/live evidence transfer. |
| `/platform/tenants/[tenantId]/configuration-transfers` | `frontend/features/platform/platform-configuration-transfer-history.tsx`, `frontend/lib/api/platform-configuration-transfer.ts` | History-only projection of immutable configuration-transfer run metadata; requires `audit.read` and contains no copy/apply controls. |
| `/platform/tenants/[tenantId]/audit` | `frontend/features/platform/platform-audit-log.tsx` | Normalized tenant-scoped immutable access/Business/integration/scheduling/transfer/operations audit projection. |
| `/s/[slug]` | `landing/app/s/[slug]/page.tsx` | Canonical public slug-scoped safe catalog from V71 JSON projection; no base-table reads or booking confirmation. The duplicate frontend route only redirects to this configured origin. |
...
| `/dashboard/staff` | `frontend/app/dashboard/staff/page.tsx` | `frontend/features/business/tenant-business-page.tsx`, `frontend/features/business/business-staff.tsx` | Tenant Business staff profile, AI-bookable state, canonical service eligibility, and archive flow. The older `staff-dashboard.tsx` calendar composition is not mounted by the active route; Platform Staff owns the normalized technical weekly schedule/time-off composition. |
| `/dashboard/settings` | `frontend/app/dashboard/settings/page.tsx` | `frontend/features/business/tenant-business-page.tsx`, `frontend/features/business/business-settings.tsx` | Tenant Business profile, canonical hours, and public page only. Provider credentials, scheduling-authority switching, internal-calendar configuration, and operations health remain in Platform Integrations, Scheduling, and Operations. |
...
- Dashboard shells and fixed product attribution: `frontend/components/layout/app-shell.tsx`,
  `frontend/components/layout/platform-app-shell.tsx`, and
  `frontend/components/layout/powered-by.tsx`.
...
- Marketing hosts/routes: `landing/proxy.ts`, `landing/lib/host-routing.ts`,
  `landing/app/page.tsx`, `landing/app/vi/page.tsx`,
  `landing/app/pricing/page.tsx`, and `landing/app/vi/pricing/page.tsx`.
- Marketing composition and registration form:
  `landing/components/marketing/*`, `landing/lib/marketing-content.ts`, and
  `landing/lib/registration-contract.ts`.
- Versioned pricing source of truth: `landing/lib/pricing-catalog.ts`.
- Marketing contract tests: `landing/lib/marketing-content.test.ts`,
  `landing/lib/marketing-template-parity.test.ts`,
  `landing/lib/pricing-catalog.test.ts`, and
  `landing/lib/registration-contract.test.ts`.
...
- Marketing brand assets and approved-template checksum evidence:
  `landing/public/brand/*` and
  `landing/lib/marketing-template-parity.test.ts`; public-salon visual asset:
  `landing/public/images/salon-hero.png`.
...
- Phase 5H conversation-policy owner: `backend/modules/scheduling.Service`
  ...
  unchanged. Normalized Platform Scheduling owns booking-mode selection beside
  authority through `backend/modules/scheduling_behavior`; the two mutations
  remain independent. Appointments owner review owns target-authority and non-
...
- Owner-notification delivery owner: V56, V90 claim atomicity,
  `backend/modules/notification_delivery`, and
  `backend/modules/notification_twilio`. The provider-neutral processor owns
  bounded single-winner claim/lease/dispatch fencing, immutable attempts/events/actions,
...
- Protected identity operator owner: `backend/cmd/platform-access/main.go`; it
  can replace only the login email of an active Tenant owner while preserving
  the immutable identity and salon relationships, revoking refresh sessions,
  and recording a replay-safe mutation. It separately creates one dedicated
  Platform identity from an owner-only password file while no active Platform
  Admin exists, supports exact replay, and never promotes a Tenant identity.
  It also exposes an operator-only exact-single-active-Admin password recovery
  that leaves email/RBAC/Tenant ownership/delegation unchanged, revokes refresh
  sessions, and writes its bounded result only to a new private file.
  The separate Tenant Owner recovery requires one exact salon/email ownership
  pair, changes only the active Tenant principal's bcrypt password, revokes
  refresh sessions, and writes a credential-free bounded result to a new
  private file; backend exists, while HTTP and UI do not exist.
- Sample-test provisioning owner: `backend/sampledata` and
  ...
  guard own reset/refusal policy. `backend/internal/config` separates the
  Compose-owned physical deployment from the behavior profile, and
  `deploy/local-backend-integration-test.sh` keeps integration writes in
  disposable databases. The `live` profile must never invoke the fixture.
...
- Data owner: Platform schema v10 plus v9/v8 upload compatibility and guarded
  ...
  v10 and compatibility-v9 `integration_providers` contain only persisted salon-scoped source rows,
...
- Landing owner: `landing/app/s/[slug]/page.tsx` and `landing/lib/api.ts`;
  `frontend/app/s/[slug]/page.tsx` redirects to the configured canonical
  public-salon origin.
...
- Frontend consumer:
  `frontend/lib/api/conversation-scheduling-evidence.ts` and
  `frontend/features/dashboard/calls-dashboard.tsx`.
- Detail-read boundary: `backend/modules/conversation/detail_read_error.go` and
  `repository.go` normalize supported legacy JSON shapes into one stable
  projection, expose safe `detail_warnings` for omitted incompatible optional
  children, and classify real read failures by section. The shared Calls UI
  keeps list/readiness state available and retries only the selected detail.
  V2 dynamic call-session routes use Fiber's GUID constraint so static Calls
  resources cannot be consumed as `session_id` values; `voice/routes_test.go`
  verifies the Conversation-then-Voice production registration order.
...
| scheduling load, concurrency harness, owner_manual replay load, transition CAS, internal party create load, external slot commit load, fake provider dispatch, conflict loser dispatch, unknown claim retention, reschedule extension race, cancel rebook race, pooled resource conflict, authority switch commit race, duplicate appointment, orphan evidence, expected conflict, p50, p95, p99, claim latency, throughput, DB pool, replica pool, non-production attestation, dedicated load database, migration checksum fingerprint, production capacity proof | `docs/operations/scheduling-load-harness.md`, `backend/cmd/scheduling-load-harness/main.go`, `backend/internal/schedulingload` | `docs/production-readiness-checklist.md`, scheduling owner repositories/services, `app_schema_migrations`, opt-in fresh-PostgreSQL integration test and sanitized schema-v2 report |
| operations health, worker heartbeat, job run, stale job, abandoned run, distributed worker lease, JOB_LEASE_EXPIRED, JOB_PANIC, JOB_RUN_FAILED, queue backlog, oldest queued, dead letter count, external_slot_claims_pre_dispatch, external_slot_claims_unknown, operations/status, V57, V86 | `docs/operations/operations-health.md`, `backend/modules/operations_health`, `backend/cmd/worker/scheduler.go` | V57 heartbeat/run tables and constraints, V86 claim queue metrics, worker job registration, authenticated route, Settings operations-health component/API/types, scheduler and PostgreSQL integration tests |
...
| scheduling behavior, booking mode UI, AI booking mode, pending approval, confirmed booking, disabled scheduling, effective behavior, booking mode version, SCHEDULING_BEHAVIOR_VERSION_CONFLICT, SCHEDULING_BEHAVIOR_INCOMPATIBLE | `backend/modules/scheduling_behavior`, `backend/modules/scheduling`, `frontend/features/platform/platform-scheduling-authority-control.tsx`, `frontend/lib/api/scheduling-behavior.ts` | `salon_settings.booking_mode`, `technical_resource_versions` `ai_receptionist/policy`, shared scheduling fence, authority × mode matrix, actual-actor technical actions/events, Platform `technical.read|write`, conversation reviewed-policy fence, configuration-transfer version trigger, service/repository/RBAC/frontend contract tests |
...
| Atomic Slot Commit, double booking, simultaneous calls, millisecond race, external slot claim, external_slot_claim_required, external_slot_claims, external_slot_claim_intervals, capability evidence, buyer_write, seller_write, booking_write_capability_version, scheduling-capability re-evaluate, reconnect_required, evidence_current, atomic_create_no_overlap, atomic_reschedule_no_overlap, concrete_staff_assignment, half-open range, sorted advisory lock, GiST exclusion, 23P01, SLOT_COMMIT_CONFLICT, SLOT_CLAIM_IN_PROGRESS, SLOT_OUTCOME_UNKNOWN, dispatch_started, dispatched_unknown, claim release, reschedule claim swap, direct Square Dashboard race, V86, V87 | `docs/scheduling-authority.md`, `docs/pos-adapter-layer.md`, `backend/modules/booking/external_slot_claim.go`, `backend/modules/booking/external_slot_claim_repository.go`, `backend/modules/pos/repository_scheduling_capability.go`, `backend/migrations/V86__external_atomic_slot_commit.sql`, `backend/migrations/V87__square_scope_bound_booking_capability.sql` | booking service/repository and handler mappings; scoped provider capabilities in `backend/modules/pos/types.go`; buyer-only Square single-create readiness/re-evaluation in `backend/modules/pos_square`; Platform Integrations safety panel; conversation refresh in `service_booking_flow.go`; authority-switch blocker; operations-health queues; V86/V87 static and real PostgreSQL concurrency tests plus schema-v2 scheduling-load harness |
...
| Platform Transfer, configuration transfer, tenant-to-tenant, JSON upload, schema v10, schema v9 compatibility, schema v8 compatibility, scoped v7 content pack, v7-to-v8 adaptation, included_sections, source fingerprint, target fences, stale preview, action replay, active_pos_provider report-only, local_business_hours, provider-managed hours, transfer run, V77 | `backend/modules/config_transfer/platform_*`, `backend/modules/config_transfer/types.go`, `backend/migrations/V77__platform_configuration_transfer.sql`, `backend/modules/scheduling/fence`, `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/lib/api/platform-configuration-transfer.ts`, `docs/lotus-investor-demo-consultation-pack-v7.json`, `docs/lotus-investor-demo-consultation-pack-v8.json` | Platform tenant Transfer tab; per-section source/target capability checks; v10 export with v9/v8 upload compatibility plus guarded content-only v7 canonicalization; file-derived UI scope; atomic canonical writes and Business/Technical/transfer audit; exact source/target/authority fence; no authority/provider/secret/connection/operational transfer; legacy Tenant/onboarding routes unmounted |
| party booking, group booking, two people, multi-guest, multi-service, split booking, aggregate availability, all-or-none, no sequential provider children, external party fail closed, partial child, party request, party service correction, guest_ref, guest_reference, quantity, sort_order, party_size, guest invariant, party_service_guest, party_service_operation, party_service_source, party request tenant mismatch, cross-salon session, V62 | `backend/modules/conversation/service_party.go`, `service_availability.go`, `service_booking_flow.go`, `repository.go`, `types.go`, `party_booking_request_fidelity_test.go`, `party_tenant_integrity_integration_test.go`, `manleai_calendar_party_scheduling_test.go` | structured `PartyPlan` guest references and ordered `guest_service_requests` JSONB, V62 composite salon/session FK and tenant-filtered hydration/redaction, `backend/modules/scheduling_manleai_calendar/execution_repository_aggregate.go`, V50 guards/tests, external party request-only handoff with zero provider child writes, Appointments internal create, Calls UI per-guest owner-review rows with legacy fallback, party golden tests |
...
| Twilio Voice, tenant-bound route, route_id, voice_inbound_number, wrong tenant, AccountSid mismatch, CallSid replay, exact webhook URL, live_verified, shared Voice route, stream status To missing | `backend/modules/integration_config`, `backend/modules/voice/twilio_route.go`, `backend/modules/voice/technical.go`, `backend/modules/voice/repository.go`, `backend/modules/voice_twilio`, `backend/internal/twiliowebhook`, `backend/internal/twiliovoice`, `backend/migrations/V83__twilio_voice_tenant_route_expand.sql` | Platform Integrations Twilio card, unbound-only provider locators/RLS, bound-tenant direct CallSid lookup without re-discovery, official signature verifier, conversation session idempotency/route fence, V83 tests, operational migration and later contract release |
| OpenAI STT, TTS, realtime, model, voice, tenant-bound, credential revision, duplicate API key, openai_public, managed destination, runtime verification, stale verification, capability probe, guarded reply, background noise, background-noise handling, automatic, strong noise rejection, false transcript, transcript logprob, repeated progress reply, stale recovery reply, caller speaking over TTS, first-byte timeout, terminal reply repeated, spoken fact mismatch, clipped first syllable, stuttered TTS startup, startup audio buffer, realtime transport fallback | `backend/internal/openairuntime`, `backend/modules/integration_config`, `backend/modules/voice_openai`, `backend/modules/openai_runtime_verification`, `backend/modules/voice`, `backend/modules/voice_twilio/handler.go`, `backend/migrations/V84__openai_tenant_bound_runtime.sql` | Platform Integrations OpenAI card, strict resolver/transport, verification worker/RLS/status, conversation runtime, realtime event timeline, rollout runbook, voice tests |
...
| integration config, provider secrets, Platform Integrations, active provider config, exact ErrNotFound, malformed settings, decrypt failure, stored secret empty, fail closed | `backend/modules/integration_config`, `/platform/tenants/:tenant_id/integrations`, `/api/v2/platform/tenants/:tenant_id/integrations` | runtime resolver and repository scanner first; the exact enabled stored row owns settings/secrets for Square, Twilio, and OpenAI; Tenant UI has no provider configuration surface; no provider runtime inherits process configuration; legacy `/technical` APIs are compatibility only |
...
| public catalog, published slug, public services, landing page, staff privacy, public readiness, stale published page, call to request, authority-aware catalog, publish fence, public base-table leak, V71 | `backend/modules/public_catalog`, `backend/modules/salon/repository.go`, `backend/migrations/V71__public_catalog_projection_boundary.sql`, `landing/app/s/[slug]/page.tsx`, `landing/lib/api.ts` | Business Settings public catalog card, scheduling authority/version, RLS integration test |
| marketing website, pricing, tenant registration, registered request, registration request, provisioning draft, owner invitation, ai.knasoftware.com, tenant-registration-contact-v1, V85 | `landing/components/marketing/*`, `landing/lib/pricing-catalog.ts`, `landing/lib/registration-contract.ts`, `backend/modules/tenant_registration`, `backend/modules/tenant_provisioning`, `backend/migrations/V85__tenant_registration_and_provisioning.sql`, `frontend/features/platform/tenant-registration-*`, `frontend/lib/api/tenant-registration-routes.ts`, `frontend/lib/api/tenant-registration-types.ts` | marketing home/pricing and bilingual form; durable pre-tenant review aggregate; Admin/Ops queue and draft; Admin-only `owner_manual` provisioning, explicit Tenant identity selection, hashed invitation, terminal retention, PostgreSQL replay/rollback coverage, and runtime-role RLS coverage |
...
| SaaS tenant, multi-tenant, ActorContext, principal_scope, immutable principal realm, tenant membership, Owner revoke, Owner reactivate, tenant_owner, tenant_business_manager, Tenant login rename, Tenant Owner credential recovery, demo identity bootstrap, Platform Admin credential recovery, rotate single admin password, platform_admin, Platform Ops, platform_ops, create Platform user, edit Platform user, direct password, Platform roles page, tenant Access tab, temporary Ops authorization, support expiry, Platform Services, Platform Calls, Platform AI Training, Platform Transfer, salon assignment, delegated capability, PII grant, sensitive data access, customers scope, calls scope, appointments scope, notifications scope, access action, access audit, Technical tab, Transfer tab, Operations tab, runtime role, RLS, system_salon_id, provider locator, worker tenant binding, worker discovery, worker claim atomicity, duplicate claim, flaky E.164 fixture, composite call tenant key, expand contract, contract preparation, strict RLS contract, policy catalog audit, Calendar actual actor, V64, V74, V75, V76, V77, V78, V79, V80, V90, V91, last admin | `backend/modules/access`, `backend/modules/business`, Platform handlers, `backend/modules/config_transfer/platform_*`, `backend/modules/tenant_runtime`, `backend/internal/database`, `backend/internal/databasecontext`, `backend/internal/middleware/auth.go`, `backend/migrations/V64*` through `V91*`, `backend/cmd/platform-access/main.go`, `deploy/production-demo-identity-bootstrap.sh`, `deploy/production-platform-admin-credential-recovery.sh`, `deploy/production-tenant-owner-credential-recovery.sh`, `.github/workflows/ci-cd.yml`, `.github/workflows/production-demo-identity-bootstrap.yml`, `.github/workflows/production-platform-admin-credential-recovery.yml`, `.github/workflows/production-tenant-owner-credential-recovery.yml`, `frontend/features/platform/access-console.tsx`, `frontend/features/platform/tenant-access-console.tsx`, `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/features/platform/access-ui.tsx`, `frontend/lib/api/access-contract.ts`, `frontend/lib/api/session-contract.ts` | immutable identity realms; current principal plus exact-route Tenant membership; replay-safe Tenant login replacement preserves user ID and salon ownership while revoking refresh sessions; separate first-Admin bootstrap never promotes Tenant identities; exact-single-active-Admin and exact-salon-active-Tenant-Owner recovery change only the bcrypt credential and session/version fences with credential-free audit; Platform Admin direct control-plane access; exact-salon baseline and time-bounded Admin grants for Ops; direct Platform user lifecycle with password hashing/session revocation; Owner suspension without ownership transfer; per-section transfer authorization; actual-actor audit including V91 Calendar config/activation/exception events; version/idempotency; V78 system tenant context; V79 worker contract preparation and call-child tenant keys; V80 strict provider/worker base-policy matching; V90 live-row eligibility rechecks plus digit-only randomized E.164 fixtures for every affected ranked worker claim; last-admin safety; RLS parity |
| deployment environment, behavior profile, DEPLOYMENT_ENV, APP_ENV, LANDING_API_BASE_URL, NEXT_PUBLIC_API_BASE_URL, Compose service origin, local production simulation, restart-prod-sim, isolated integration database, TEST_DATABASE_URL, sample data, sample_test, Lotus Nails Studio, fixture migration, sample_data_migrations, sample Platform Admin, sample Platform Ops, pre-live data, data profile, DEPLOY_DATA_PROFILE, SAMPLE_DATA_ENV_B64, SAMPLE_TEST_RESET_RELEASE_TAG, migration checksum mismatch, migration preflight, V73, V74, data classification, principal scope | `backend/internal/config/config.go`, `backend/sampledata`, `backend/cmd/sample-data/main.go`, `backend/migrations/V73__sample_data_classification.sql`, `backend/migrations/V74__principal_scope_isolation.sql`, `docker-compose.yml`, `docker-compose.prod.yml`, `deploy/local-restart.sh`, `deploy/local-environment-contract*.sh`, `deploy/local-backend-integration-test.sh`, `deploy/postgres-migration-preflight.sh`, `deploy/postgres-sample-target-preflight.sh`, `deploy/postgres-data-profile-guard.sh`, `landing/lib/http-origin.ts`, `landing/lib/api.ts`, `landing/lib/config.ts`, `.github/workflows/ci-cd.yml` | Compose fixes physical deployment and owns the server-only landing API origin; local validation derives the service host from rendered Compose state and verifies runtime `/healthz`, while the browser origin remains separate. Behavior inherits the physical environment, local may simulate production, and production cannot downgrade; local integration tests use disposable databases; normal migrations stay fixture-free; local restart and protected pre-live deploy provision/replay the separate fixture; backup-first exact-tag reset only for incompatible `sample_test`; `live` refuses reset/sample residue and preserves bootstrap |
...
| production tenant catalog audit, production service count, services empty, services unavailable, services matching active provider, ManleAI Calendar config row, calendar activation version, calendar setup counts, Platform Admin Services authorization, read-only tenant SQL, no catalog PII | `.github/workflows/production-tenant-catalog-audit.yml`, `deploy/production-tenant-catalog-audit.sh` | protected `production` environment, exact UUID tenant input, `salons`, `salon_settings`, `services`, service categories/aliases, ManleAI Calendar config/local-hours/service-policy/staff-weekly-period tables, V76 authorization functions, `docs/deployment.md`, GitHub Actions audit log |
| production Platform Admin RBAC matrix audit, complete capability matrix, services.write production permission, Platform Admin PII decision, RBAC drift, unexpected Admin capability, read-only authorization SQL | `.github/workflows/production-platform-admin-rbac-audit.yml`, `deploy/production-platform-admin-rbac-audit.sh` | protected `production` environment, exact UUID tenant input, 20 registered capabilities, four compatible PII decisions, V76 authorization functions, `docs/deployment.md`, GitHub Actions audit log |
| production demo identity, Tenant login rename, preserve Lotus tenant, Platform Admin bootstrap, generated Admin password, identity action replay | `.github/workflows/production-demo-identity-bootstrap.yml`, `deploy/production-demo-identity-bootstrap.sh`, `backend/cmd/platform-access/main.go`, `backend/modules/access/tenant_email.go` | protected `production` environment, `users`, `salons.owner_user_id`, `refresh_tokens`, `platform_role_assignments`, `access_control_actions`, `access_control_events`, `docs/deployment.md` |
| production Platform Admin credential recovery, rotate password, lost admin password, exact single active admin, encrypted credential handoff, RSA-OAEP-SHA256, AES-256-GCM | `.github/workflows/production-platform-admin-credential-recovery.yml`, `deploy/production-platform-admin-credential-recovery.sh`, `deploy/production-platform-admin-credential-recovery-test.sh`, `deploy/encrypt-platform-admin-credential-handoff.mjs`, `deploy/encrypt-platform-admin-credential-handoff-test.sh`, `backend/cmd/platform-access/main.go`, `backend/modules/access/platform_admin_recovery.go` | protected `production` environment; exact running release and account aggregate; password-only serializable mutation; refresh-token revocation; assignment-version fence; credential-free immutable audit; tested one-day hybrid-encrypted artifact; runner/VPS plaintext cleanup |
| production Tenant Owner credential recovery, lost Tenant password, exact salon owner, rotate tenant owner password | `.github/workflows/production-tenant-owner-credential-recovery.yml`, `deploy/production-tenant-owner-credential-recovery.sh`, `deploy/production-tenant-owner-credential-recovery-test.sh`, `backend/cmd/platform-access/main.go`, `backend/modules/access/tenant_password_recovery.go` | protected `production` environment; exact running release and two-account aggregate; exact salon/email active-Tenant-owner fence; password-only serializable mutation; refresh-token revocation; credential-free immutable audit and result; runner/VPS plaintext cleanup after operator-controlled handoff |
