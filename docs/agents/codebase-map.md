...
| `/platform/tenants/[tenantId]/technical` | `frontend/features/platform/technical-integration-settings.tsx`, shared `frontend/features/integrations/square-integration.tsx` provider panel, `frontend/lib/api/integration-config-contract.ts`, `technical-scheduling-settings.tsx` | Platform-only Square/Twilio/OpenAI and scheduling controls. Twilio exposes editable inbound E.164/Account SID/Auth Token/public base/enablement, immutable route ID, computed read-only URLs, blockers, and separate configured/live-verified evidence. OpenAI exposes managed destination, local runtime blockers, credential revision/uniqueness, explicit paid verification action, per-capability evidence, safe failure/stale/partial states, and saved versus fresh live-verified truth. No standalone provider routing/verification page. |
