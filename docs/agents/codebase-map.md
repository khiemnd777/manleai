...
| Integration config | `backend/modules/integration_config/*` | Authenticated salon-scoped Square/Twilio/OpenAI runtime settings and encrypted secrets. Twilio owns immutable integration-row route UUID, active unique inbound E.164, routing enablement, masked Account SID hint, computed tenant URLs, provider-only UUID locator, exact-tenant RLS reload, number-conflict handling, owner-SMS settings, and strict messaging/audio resolvers. OpenAI owns the fixed destination profile, purpose-separated unique credential HMAC identity, monotonic credential revision, shared runtime validator, typed duplicate-key conflict, and strict `ResolveOpenAIRuntimeConfig`. Only Square retains exact-missing legacy bootstrap; Twilio/OpenAI never inherit process config; repository/settings/decryption failures remain fail closed. | integration-config tests, V83/V84 migration tests, Voice/Square caller propagation tests |
