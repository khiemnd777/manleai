...
| production tenant catalog audit, production service count, services empty, services unavailable, services matching active provider, Platform Admin Services authorization, read-only tenant SQL, no catalog PII | `.github/workflows/production-tenant-catalog-audit.yml`, `deploy/production-tenant-catalog-audit.sh` | protected `production` environment, exact UUID tenant input, `salons`, `salon_settings`, `services`, service categories/aliases, V76 authorization functions, `docs/deployment.md`, GitHub Actions audit log |
