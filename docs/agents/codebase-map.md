...
| `/platform`, `/platform/tenants` | `frontend/components/layout/platform-app-shell.tsx`, `frontend/components/layout/powered-by.tsx`, `frontend/features/platform/tenant-directory.tsx` | Platform-only shell and tenant directory; the shared shell renders fixed KNASOFTWARE attribution after page content and links it to `https://www.knasoftware.com` in a new tab |
...
| `/platform/tenants/[tenantId]/technical` | `frontend/features/platform/technical-integration-settings.tsx`, shared `frontend/features/integrations/square-integration.tsx` provider panel, `frontend/lib/api/integration-config-contract.ts`, `technical-scheduling-settings.tsx` | Platform-only Square/Twilio/OpenAI and scheduling controls. The existing Square connection panel owns V87 scheduling-safety badges/operation rows, buyer/seller write mode, expiring evidence/blocker state, reconnect, exact-version re-evaluation, and the V88 reviewed initial active-provider action; connection/sync never selects the provider implicitly. No checkbox, manual capability override, or standalone capability table/page exists. Twilio exposes editable inbound E.164/Account SID/Auth Token/public base/enablement, immutable route ID, computed read-only URLs, blockers, and separate configured/live-verified evidence. OpenAI exposes managed destination, local runtime blockers, credential revision/uniqueness, explicit paid verification action, per-capability evidence, safe failure/stale/partial states, and saved versus fresh live-verified truth. No standalone provider routing/verification page. |
| `/platform/tenants/[tenantId]/transfer` | `frontend/features/platform/platform-configuration-transfer.tsx`, `frontend/lib/api/platform-configuration-transfer.ts` | Platform-only tenant-to-tenant, v10/v9/v8 JSON, or scoped content-only v7 transfer; destination implied by route; file-derived portable scope and visible v7 adaptation; preview/apply/stale/conflict/replay states and safe run history; no authority/provider credential identity/secret/live evidence transfer |
...
| integration config, provider secrets, Platform Technical, active provider config, exact ErrNotFound, malformed settings, decrypt failure, stored secret empty, fail closed | `backend/modules/integration_config`, `/platform/tenants/:tenant_id/technical`, Platform technical integration/status APIs | runtime resolver and repository scanner first; the exact enabled stored row owns settings/secrets for Square, Twilio, and OpenAI; Tenant UI has no provider configuration surface; no provider runtime inherits process configuration |
