...
| Voice provider-neutral | `backend/modules/voice/*` | Owner-scoped, authority/version-fenced voice status with separate `phone_answering`, `request_capture`, and `automated_booking` dimensions; Twilio phone answering now requires the stored tenant-bound route/inbound E.164/Account SID/Auth Token/public HTTPS base rather than `salons.phone`; `twilio_route.go` owns the server-only resolved/verified route proof, immediate provider tenant binding, route/number/AccountSid/CallSid agreement, no-side-effect rejection, and current-fingerprint live evidence; `technical.go` owns Platform-safe configured-versus-live routing status; target readiness remains owned by the selected scheduling executor; also owns semantic-contract verification, bounded read-only semantic evaluation, guarded model-reply conversion, safe provider diagnostics, STT/LLM/TTS/realtime and streaming-speech interfaces, speech turns, and public audio delivery through metadata-first bounded HMAC capabilities | `backend/modules/voice/*_test.go`, `backend/modules/voice_twilio/handler_test.go`, `backend/modules/conversation/repository_concurrency_integration_test.go`, `backend/migrations/v83_*_test.go` |
