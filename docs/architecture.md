...
The live voice layer is split into `modules/voice`, `modules/voice_twilio`, and provider-specific AI adapter modules such as `modules/voice_openai`. `modules/voice` owns provider-neutral DTOs, runtime interfaces for telephony/STT/LLM/TTS/realtime providers, salon phone routing, dashboard readiness, short-lived TTS audio output persistence, and webhook event audit writes. `modules/voice_twilio` owns Twilio-specific request signature validation, webhook form mapping, recording fetches, TwiML responses, and Twilio Media Streams WebSocket framing. `modules/voice_openai` owns OpenAI API URLs, payloads, response parsing, model configuration, and Realtime WebSocket sessions. GA Realtime requests transcription log probabilities and applies a profile-aware admission policy: missing or invalid confidence metadata fails closed, low mean or low-tail confidence is rejected, and implausible transcript density relative to VAD duration is rejected without entering conversation state. Noisy and balanced input also use near-field noise reduction. Backend-approved realtime replies use a bounded FIFO and application request IDs; provider response IDs bind `response.created`, audio, cancellation, and terminal events to the active reply so stale or interrupted audio cannot be spoken as the next turn. The barge-in guard applies before cancellation in both legacy and GA branches. Backend progress copy is delayed and limited to one occurrence per call. Each GA output request is isolated from the default provider conversation, and audio remains buffered until a completed output transcript matches the backend-approved operational facts after deterministic normalization of punctuation, contractions, spoken numbers, times, dates, and digit sequences. A missing output transcript or fact mismatch never releases the audio, but the terminal stream redirect resumes the last backend-approved prompt through recording or gather transport instead of implying an owner handoff while the session is still active. PII-free timing and validation diagnostics are available in the Calls realtime timeline; transcript and audio bodies are not exposed. Operational facts bypass style rewriting, while explicitly style-only copy may use the guarded reply generator. Twilio speech gathering, recording mode, realtime stream transport, TwiML speech output, and audio playback are treated as provider adapter behavior, not conversation-engine logic. Accepted completed realtime transcripts still enter the same conversation engine and booking service; OpenAI Realtime does not call Square or confirm bookings on its own.
