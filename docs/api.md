...
Returns one conversation session with transcript messages and the latest handoff request when present. Booking state includes `requested_date` when the customer has provided a day but not a specific time, and `requested_start_time` only after a concrete start time or offered slot is selected. `dialog_state` is a versioned operational state object containing the conversation phase, pending typed clarification, last reversible draft mutation, bounded no-progress count, and final-review flags. Transcript messages may include backend metadata for turn diagnostics such as slot state before/after, event keys, guardrail outcomes, typed conversation act, answer source, source reason, router intent, source record IDs, answer-context cache hit state, and next required booking field.

Production-created sessions use `dialog_state.review_required=true`. When every booking field is complete, the runtime returns `phase=review` and asks the caller to review the draft. A later explicit authorization sets `review_accepted=true` and may invoke the booking service. Review acceptance is not booking confirmation.
