...
- [x] Make voice readiness service/staff counts use the same provider/location/generation/link/version/activity fence as conversation context, and expose typed service-guidance readiness with catalog, toggle, and ready-profile evidence.
- [x] Route freeform service-menu requests recognized by `guidance_action=service_catalog` only through the active bookable catalog, and constrain no-provider recovery parsing to the immediately active state-owned choices instead of general keyword intent matching.
...
- [x] Add a bounded owner-scoped semantic-evaluation endpoint plus a reproducible schema-v2 1,000-scenario single-turn contract corpus using the same guidance/full split as runtime, 100-record source/state/hardcoding review, offline validator that reports zero model executions/passes, and a report-required live scorer hard-capped at 12 canaries that cannot create sessions, invoke POS tools, or confirm a booking.
- [x] Add a separate 50-execution direct-model feasibility pilot from 45 directly authored real nail-salon situations, including five phone/simulator pairs; resolve the active OpenAI model and encrypted key strictly from `salon_integration_configs`, reuse production recognition/conversation/reply guardrails over isolated fixtures, block conversation/POS writes, checkpoint every paid request, enforce an explicit model-call ceiling, retain token usage and final replies, and run 10 actual five-output review rounds whose five dimensions must each score at least 4/5. This checklist item covers harness readiness only; no repository test or corpus generation claims that the stored model has passed the paid pilot.
