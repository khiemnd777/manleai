...
- [x] Fail runtime Square/Twilio/OpenAI configuration resolution closed on missing rows, repository failures, malformed persisted settings, disabled state, or decryption errors; keep stored enabled state and credentials authoritative; make all three runtime/read paths database-only; and prevent dashboard secret-source fallback.
...
- [x] Derive tagged-deploy release evidence from the immutable GitHub tag and actor, record the repository-owned expand/contract rollback policy, and require no per-tag GitHub variable update.
...
- [x] Keep image rollback distinct from database recovery: automated failure handling restarts prior images under the normal release path's expand/contract policy, never rewinds PostgreSQL, and records that no database restore occurred.
...
- [x] Own V86/V87/V88 safety, strict Square tenant identity and initial activation, scope-bound Square capability, real PostgreSQL external slot-claim concurrency, and schema-v2 scheduling-load tests in the release manifest; require their isolated PostgreSQL identity in release mode so missing database coverage fails instead of skipping.
...
- [x] Keep `build-images` and `deploy` dependent on the release gate and preserve the tag-only, pre-deploy backup, repository-owned expand/contract rollback policy, healthcheck, and rollback gates without per-tag GitHub variable updates.
...
- [x] Fence final conversation review to both `booking_mode` and selected scheduling authority; require a new full review after either changes; preserve ordered party/service/staff/requested-time data for pending requests while stripping execution proof; persist the optional target authority through V55 without backfilling legacy NULL requests; expose booking mode beside authority in normalized Platform Scheduling with independent versioned/audited mutation and effective-behavior evidence, and expose target/non-reservation evidence in Appointments owner review.
