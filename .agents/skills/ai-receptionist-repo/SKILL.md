...
- Supported group or party booking requests can call availability and booking tools after the conversation resolves party size and guest service counts into catalog-backed booking segments. They must not use confirmed appointment wording unless POS booking succeeds; owner-review `party_booking_requests` are exception records, not the default path.
