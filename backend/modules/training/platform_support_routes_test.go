...
func TestPlatformTrainingGuardFailsClosedBeforeDomainHandlerWhenAuditFails(t *testing.T) {
	app := fiber.New()
	reached := false
	app.Get("/platform/tenants/:id/knowledge-items", requirePlatformCapability(failingSupportAudit{}, access.CapabilityTrainingRead), func(c *fiber.Ctx) error {
		reached = true
		return c.SendStatus(fiber.StatusOK)
	})

	response, err := app.Test(httptest.NewRequest(http.MethodGet, "/platform/tenants/salon-1/knowledge-items", nil))
	if err != nil {
