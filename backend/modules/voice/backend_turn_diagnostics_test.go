...
func TestBackendTurnDiagnosticsWhitelistsSafeInterpreterFailureFields(t *testing.T) {
	...
	diagnostics.Record(conversation.TurnTiming{
		...
		Attributes: map[string]string{
			...
			"turn_semantic_contract":              conversation.TurnSemanticContractGuidance,
	...
	if snapshot["turn_semantic_contract"] != conversation.TurnSemanticContractGuidance || snapshot["turn_interpreter_http_status_class"] != "5xx" || snapshot["turn_interpreter_request_id"] != "req_safe_123" ||
