...
func TestMigrationFilesOrderV82AfterV81(t *testing.T) {
	files, err := loadMigrationFiles()
	if err != nil {
		t.Fatalf("load migration files: %v", err)
	}
...
func TestMigrateAppliesForwardMigrationOnceWithoutChangingAppliedChecksums(t *testing.T) {
	...
	if err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM app_schema_migrations WHERE version = '80'`).Scan(&count); err != nil {
		t.Fatalf("load V80 record: %v", err)
	}
	if count != 1 {
	...
	if err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM app_schema_migrations WHERE version = '82'`).Scan(&count); err != nil {
		t.Fatalf("load V82 record: %v", err)
	}
	if count != 1 {
