import { LoginForm } from "@/features/auth/login-form";
import { Card } from "@/components/ui/card";

export default function LoginPage() {
  return (
    <main className="flex min-h-screen bg-shell">
      <section className="hidden flex-1 items-center justify-center bg-ink px-10 text-white lg:flex">
        <div className="max-w-lg">
          <div className="mb-8 inline-flex rounded-full border border-white/20 px-3 py-1 text-sm text-white/80">
            Authority-aware scheduling calendar
          </div>
          <h1 className="text-5xl font-bold leading-tight">POS Calendar</h1>
          <p className="mt-5 text-lg leading-8 text-white/72">
            View day, week, month, and agenda schedules with Square-backed booking actions and sync warnings.
          </p>
          <div className="mt-10 grid gap-3 text-sm text-white/80">
            <div className="rounded-md border border-white/15 p-4">
              Appointments are confirmed only after the active POS returns a booking ID.
            </div>
            <div className="rounded-md border border-white/15 p-4">
              Sync issues stay visible on each affected calendar item.
            </div>
          </div>
        </div>
      </section>
      <section className="flex flex-1 items-center justify-center px-5">
        <Card className="w-full max-w-md">
          <div className="mb-6">
            <div className="text-sm font-semibold text-brand">Calendar view</div>
            <h2 className="mt-2 text-2xl font-bold text-ink">Sign in</h2>
            <p className="mt-2 text-sm leading-6 text-muted">
              Use the same admin account as the owner dashboard.
            </p>
          </div>
          <LoginForm />
        </Card>
      </section>
    </main>
  );
}
